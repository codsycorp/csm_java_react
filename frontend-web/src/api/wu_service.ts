...
export function getCategoryLabel(code?: string): string {
  ...
  return (CATEGORY_LABELS_DYNAMIC[k] || CATEGORY_LABELS[k] || k);
}
  ...
  try {
    ...
    if (ssrCategories && Array.isArray(ssrCategories)) {
      ...
      for (const cat of ssrCategories) {
        ...
        if (code && name) {
          CATEGORY_LABELS_DYNAMIC[code] = name;
        }
      }
...
function parseJSON<T = any>(val: any, fallback: T): T {
  ...
  try {
    ...
    // Handle double-stringified JSON (common issue with images field)
    if (Array.isArray(parsed)) {
      parsed = parsed.map(item => {
        if (typeof item === 'string') {
          try {
            // Try to parse again if it's still a JSON string
        ...
        return item;
      }).filter(item => item && (typeof item === 'string' ? item.trim().length > 0 : true));
    }
    return parsed as T;
...
export async function fetchServiceDetail(category: string, id: string) {
  ...
  const queryIndex = idStr.indexOf('?');
  if (queryIndex > 0) {
    idStr = idStr.substring(0, queryIndex);
  }
  ...
  if (!post && detailRow) {
    ...
    // Helper to get first string from thumbnail/cover (might be array or JSON string)
    ...
    post = {
      ...
      serviceType,
      thumbnail: getThumbnailString(detailRow.thumbnail) || getThumbnailString(detailRow.cover) || String(seoMeta?.og_image || ''),
...
export async function fetchRelatedServices(category: string, id: string, take = 4, page = 1, lastkey?: any, options?: { q?: string }) {
  ...
  // Nếu SSR đã trả sẵn block liên quan (đã phân trang server-side), dùng trực tiếp để tránh đọc toàn bộ danh sách
  if (Array.isArray(initialData.relatedDetailList) && initialData.relatedDetailList.length > 0) {
    const total = typeof initialData.totalRelatedCount === 'number'
      ? initialData.totalRelatedCount
      : initialData.relatedDetailList.length;
    return { success: true, data: initialData.relatedDetailList, total, totalCount: total };
  }
  if (!initialData.serviceDetailList) {
    throw new Error('SSR serviceDetailList missing: cannot derive related services');
  }
