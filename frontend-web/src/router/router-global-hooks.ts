...
export const routerBeforeEach: (reactRouter: ReactRouterType) => BlockerFunction = reactRouter => ({ nextLocation }) => {
	...
	const canonicalPath = resolveCanonicalPathFromCategories(pathnameWithoutBase, (window.__SSR_WEBSITE_CATEGORIES__ as Array<any>) || []);
	if (canonicalPath && canonicalPath !== pathnameWithoutBase) {
		reactRouter.navigate(canonicalPath, { replace: true });
		return true;
	}
