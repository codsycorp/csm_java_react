...
export function useMenu() {
	...
	const translatedMenus = useMemo(() => {
		...
		// Frontend web không có role admin; chỉ giữ dev flag nếu cần debug/system nội bộ.
	...
	const processedMenus = useMemo(() => {
		// Ẩn hoàn toàn menu hệ thống (/system) với user không phải dev
		const canSeeSystemMenus = isDev;
