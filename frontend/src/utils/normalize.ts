...
function toArrayOfStrings(v: any): string[] | undefined {
  if (!v) return undefined;
  if (Array.isArray(v)) {
    // Handle nested JSON strings in array like ["\"/url\""]
    return v.map(x => {
      if (typeof x === 'string') {
        try {
  ...
  if (typeof v === "string") {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        return toArrayOfStrings(parsed);
      }
    } catch {
      // Not JSON, treat as comma/newline separated string
...
export function normalizeServiceDetail(raw: any): ServicePost {
  ...
  const getFirstString = (value: any): string | undefined => {
    ...
    if (typeof value === 'string') {
      // Try to parse if it's a JSON array string like '["url"]'
      try {
        ...
        return value; // Not JSON, use as is
      }
    }
  ...
  const thumbnail = getFirstString(raw?.thumbnail ?? raw?.image_url ?? raw?.cover);
