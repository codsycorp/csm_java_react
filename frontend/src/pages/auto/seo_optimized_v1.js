...
window.strGoogleAds = strEvent + `
  ...
  window.google_click=function(tu_khoa,link_check,time_o_lai_trang,isRunAds){
    ...
    else if(location.href.indexOf("google.com/sorry/index")!=-1||(location.href.indexOf("google.com")===-1 && location.href!==link_check))
    {
      ...
      setTimeout(function() {console.log(JSON.stringify({type:"close",tabid:tabid}));}, 60000);
    }
      ...
      if(el)
      {
        if(el.value=="")
        {
          try{
            ...
            console.error('[mouseEvent] Lỗi:', e);
          }
        }
        else
        {
    ...
    {
      const directOpenDelayMs = (!tu_khoa && link_check && location.href.indexOf("google.com") === -1) ? 5000 : 30000;
      setTimeout(function(){
        simulateRealisticUserBehavior(time_o_lai_trang);
      }, directOpenDelayMs);
      if(1*isRunAds===2) {
...
window.__proxyHost = ''; // Proxy host để dùng cho health check
window.__proxyPort = 0; // Proxy port để dùng cho health check
...
window.fnClearWebviewCache = function (webviewId, killProcess = false) {
  try {
    ...
    // Xóa browser cache
    wv.clearData({ since: 0 }, {
      appcache: true, cache: true, cookies: true,
      fileSystems: true, indexedDB: true, localStorage: true, webSQL: true
    });

    // Xóa partition files
    const partitionId = wv.dataset.partitionId;
    if (partitionId && window.hasOwnProperty("process")) {
      const delay = (typeof process !== 'undefined' && process.platform === 'darwin') ? 5000 : 3000;
      setTimeout(() => {
        if (typeof window.deletePartitionFilesCompletely === 'function') {
          window.deletePartitionFilesCompletely(partitionId);
        }
      }, delay);
    }
  } catch (err) {
    console.error('[fnClearWebviewCache] Error:', err.message);
  }
};
window.fnClearCache = function () {
  ...
  window.chrome.browsingData.remove({
    ...
    fileSystems: true, history: true, indexedDB: true, localStorage: true,
    pluginData: true, passwords: true, serverBoundCertificates: true, webSQL: true
  });
}

// Maximum time to spend retrying a single folder (30 seconds) - PREVENTS FOREVER FREEZING
// After 30s, if files still locked, we give up and mark for cleanup on next restart
const MAX_RETRY_TIME_MS = 30 * 1000;

/**
 * SIMPLE AGGRESSIVE DELETION - Single pass, no retries
 * Deletes what it can, skips locked files
 * Process will eventually release them when it terminates
 * 
 * @param {string} directoryPath - Path to delete
 * @returns {object} { deleted: boolean, skipped: number }
 */
const deleteFolderSimple = function(directoryPath) {
  if (!window.hasOwnProperty('process') || !directoryPath) return { deleted: false, skipped: 0 };
  ...
  try {
    ...
    if (fs.existsSync(directoryPath)) {
      const deleteRecursiveSimple = (dirPath) => {
        let skipped = 0;
        try {
          ...
          files.forEach(file => {
            try {
              ...
              if (stat.isDirectory()) {
                skipped += deleteRecursiveSimple(filePath);
              } else {
                try {
                  fs.unlinkSync(filePath);
                } catch (err) {
                  // File locked - just skip (process still using it)
                  skipped++;
                }
              }
            } catch (e) {
          ...
          try {
            ...
            // Directory not empty - files still locked
            if (e.code !== 'ENOTEMPTY') {
              // Some other error
            }
          }
      ...
      skippedCount = deleteRecursiveSimple(directoryPath);
      const deleted = !fs.existsSync(directoryPath);
      return { deleted, skipped: skippedCount };
    }
    return { deleted: true, skipped: 0 };
  } catch (e) {
    console.warn(`[deleteFolderSimple] Error deleting ${directoryPath}:`, e.message);
    return { deleted: false, skipped: -1 };
  }
};

const deleteFolderRecursive = function (directoryPath, retryCount = 0, maxRetries = 30) {
  if (window.hasOwnProperty("process")) {
    const fs = require('fs');
    const Path = require('path');
    
    // 🛡️ SAFEGUARD: Track đường dẫn đang retry để tránh duplicate retries
    if (!window.__deleteFolderRetryTracker) {
      window.__deleteFolderRetryTracker = new Map(); // path -> retryCount
    }
    
    const trackingKey = directoryPath;
    const tracked = window.__deleteFolderRetryTracker.get(trackingKey);
    const existingRetryCount = typeof tracked === 'number' ? tracked : (tracked?.count || 0);
    const nextRetryAt = typeof tracked === 'object' && tracked ? tracked.nextRetryAt : null;

    // Nếu đang trong thời gian chờ retry thì không chạy lại
    if (nextRetryAt && Date.now() < nextRetryAt) {
      return;
    }
    
    // Nếu đã retry quá nhiều lần cho path này, mark để xóa khi restart app
    if (existingRetryCount >= maxRetries) {
      console.warn(`[deleteFolderRecursive] ⛔ Max retries (${maxRetries}) exceeded for: ${directoryPath}`);
      console.warn(`[deleteFolderRecursive] 📝 Marking for cleanup on next app restart`);
      window.__deleteFolderRetryTracker.delete(trackingKey);
      
      // Lưu vào persistent storage để xóa khi app restart
      if (typeof window.markForDeferredCleanup === 'function') {
        window.markForDeferredCleanup(directoryPath);
      }
      return;
    }
    ...
    try {
      if (fs.existsSync(directoryPath)) {
        ...
        fs.readdirSync(directoryPath).forEach((file, index) => {
          try {
            ...
            if (fs.lstatSync(curPath).isDirectory()) {
              // recurse
              deleteFolderRecursive(curPath, retryCount, maxRetries);
            } else {
              // delete file
              try {
                fs.unlinkSync(curPath);
              } catch (unlinkErr) {
                // File bị locked - đánh dấu để retry toàn bộ directory sau
                hasLockedFiles = true;
                console.warn(`[deleteFolderRecursive] ⚠️ Cannot delete file (locked): ${curPath}`, unlinkErr.message);
                // Không throw - tiếp tục xóa các files khác
                return;
              }
            }
          } catch (e) {
            console.warn(`[deleteFolderRecursive] ⚠️ Error processing: ${Path.join(directoryPath, file)}`, e.message);
        ...
        try {
          ...
          console.log(`[deleteFolderRecursive] ✅ Deleted directory: ${directoryPath}`);
          // Thành công → xóa tracking
          window.__deleteFolderRetryTracker.delete(trackingKey);
        } catch (rmdirErr) {
          // Directory không thể xóa (có locked files hoặc không empty)
          if (hasLockedFiles || rmdirErr.code === 'ENOTEMPTY') {
            const trackedNow = window.__deleteFolderRetryTracker.get(trackingKey);
            const currentRetryCount = typeof trackedNow === 'number' ? trackedNow : (trackedNow?.count || 0);
            const firstAttemptTime = trackedNow?.firstAttemptAt || Date.now();
            const totalRetryTime = Date.now() - firstAttemptTime;
            
            // CRITICAL: Stop retrying if we've spent > 30 seconds on this folder
            if (totalRetryTime > MAX_RETRY_TIME_MS) {
              console.error(`[deleteFolderRecursive] ❌ TIMEOUT: Stopped retrying after ${(totalRetryTime/1000).toFixed(0)}s. Folder PERMANENTLY LOCKED: ${directoryPath}`);
              console.error(`[deleteFolderRecursive] ❌ ROOT CAUSE: Webview processes NOT terminating, files held by OS/process`);
              console.warn(`[deleteFolderRecursive] ℹ️ App will retry cleanup every 5 minutes automatically`);
              console.warn(`[deleteFolderRecursive] ℹ️ Or manually trigger: window.retryLockedFolderCleanup()`);
              window.__deleteFolderRetryTracker.delete(trackingKey);
              window.markForDeferredCleanup(directoryPath); // Mark for periodic retry
            } else if (currentRetryCount < maxRetries) {
              const nextRetryCount = currentRetryCount + 1;
              // Aggressive retry: 10s, 20s, 30s, 40s... then cap at 60s
              const baseDelayMs = Math.min(10000 * nextRetryCount, 60000);
              const delayMs = baseDelayMs;
              
              console.log(`[deleteFolderRecursive] 🔄 Retry ${nextRetryCount}/${maxRetries} in ${delayMs/1000}s: ${directoryPath}`);
              
              // Update tracking (đếm + thời điểm retry tiếp theo + first attempt time)
              window.__deleteFolderRetryTracker.set(trackingKey, {
                count: nextRetryCount,
                nextRetryAt: Date.now() + delayMs,
                firstAttemptAt: firstAttemptTime // Track when we FIRST started trying
              });
              
              // Schedule ONE retry (không tạo nhiều retries)
              setTimeout(() => {
                deleteFolderRecursive(directoryPath, nextRetryCount, maxRetries);
              }, delayMs);
            } else {
              // console.warn(`[deleteFolderRecursive] ⛔ Max retries exceeded for: ${directoryPath}. Locked files remain.`);
              window.__deleteFolderRetryTracker.delete(trackingKey);
            }
          } else {
            // console.warn(`[deleteFolderRecursive] ⚠️ Cannot delete directory: ${directoryPath}`, rmdirErr.message);
          }
        }
      }
    } catch (e) {
      console.error(`[deleteFolderRecursive] ❌ Error: ${directoryPath}`, e.message);
    }
  }
};

/**
 * PERSISTENT CLEANUP TRACKING - Xóa files bị lock khi app restart
 * Lưu danh sách folders/files không thể xóa (do bị lock) vào localStorage
 * Khi app khởi động lại, sẽ thử xóa lại các folders này
 */
window.markForDeferredCleanup = function(path) {
  if (!path || typeof path !== 'string') return;
  
  try {
    const key = 'deferred_cleanup_paths';
    const existingPaths = JSON.parse(localStorage.getItem(key) || '[]');
    
    if (!existingPaths.includes(path)) {
      existingPaths.push(path);
      localStorage.setItem(key, JSON.stringify(existingPaths));
      console.log(`[DeferredCleanup] 📝 Marked for cleanup on restart: ${path}`);
    }
  } catch (e) {
    console.error('[DeferredCleanup] Error marking path:', e.message);
  }
};

window.processDeferredCleanup = function() {
  ...
  try {
    ...
    if (paths.length === 0) {
      console.log('[DeferredCleanup] ℹ️ No deferred cleanup needed');
      return;
    }
    
    console.log(`[DeferredCleanup] 🧹 Processing ${paths.length} deferred paths from previous session...`);
    ...
    paths.forEach(path => {
      if (!fs.existsSync(path)) {
        console.log(`[DeferredCleanup] ✅ Already deleted: ${path}`);
        successPaths.push(path);
        return;
      }
      
      try {
        // Try force deletion using system commands on Windows
        if (process.platform === 'win32') {
          const childProcess = require('child_process');
          // Use rd /s /q for Windows force recursive delete
          childProcess.execSync(`rd /s /q "${path}"`, { timeout: 10000 });
          console.log(`[DeferredCleanup] ✅ Force deleted (Windows rd): ${path}`);
          successPaths.push(path);
        } else {
          // Unix/Mac: use rm -rf
          deleteFolderRecursive(path, 0, 5); // Only 5 retries on startup
          console.log(`[DeferredCleanup] ✅ Deleted: ${path}`);
          successPaths.push(path);
        }
      } catch (err) {
        console.warn(`[DeferredCleanup] ⚠️ Still locked: ${path}`, err.message);
        failedPaths.push(path);
      }
    });
    
    // Update localStorage - keep only failed paths for next restart
    if (failedPaths.length > 0) {
      localStorage.setItem(key, JSON.stringify(failedPaths));
      console.log(`[DeferredCleanup] 📝 ${failedPaths.length} paths still locked, will retry next startup`);
    } else {
      localStorage.removeItem(key);
      console.log(`[DeferredCleanup] ✅ All deferred paths cleaned successfully!`);
    }
    
  } catch (e) {
    console.error('[DeferredCleanup] Error processing:', e.message);
  }
};

/**
 * Manual trigger deferred cleanup - cho phép user cleanup ngay
 */
window.forceCleanupNow = function() {
  console.log('[ForceCleanup] 🚀 Manually triggering cleanup...');
  window.processDeferredCleanup();
};

/**
 * Xem danh sách files/folders đang chờ cleanup
 */
window.viewPendingCleanup = function() {
  try {
    const key = 'deferred_cleanup_paths';
    const paths = JSON.parse(localStorage.getItem(key) || '[]');
    ...
    console.log(`📋 Pending cleanup items (${paths.length}):`);
    paths.forEach((path, idx) => {
      console.log(`  ${idx + 1}. ${path}`);
    });
    
    console.log('\n💡 Tip: Call window.forceCleanupNow() to cleanup now, or restart app for auto-cleanup');
    
  } catch (e) {
    console.error('Error viewing pending cleanup:', e.message);
  }
};

// Auto-run deferred cleanup on app startup
if (typeof window !== 'undefined' && window.hasOwnProperty('process')) {
  // Delay 10s sau khi app start để đảm bảo không có locks
  setTimeout(() => {
    console.log('[DeferredCleanup] 🚀 Running startup cleanup...');
    window.processDeferredCleanup();
  }, 10000);
}

/**
 * CONFIG: WEBVIEW STORAGE CLEANUP
 * Cấu hình thời gian để xác định partition folders nào có thể xóa
 * 
 * 🔧 CÁC CẢI TIẾN ĐÃ THỰC HIỆN:
 * 1. ✅ Tăng maxRetries từ 10 → 30 lần để xóa triệt để hơn
 * 2. ✅ Giảm retry delay từ 30s → 10-60s (aggressive retry) để xóa nhanh hơn
 * 3. ✅ Tăng cleanup delay cho macOS từ 3-5s → 5-10s do macOS giữ file locks lâu hơn
 * 4. ✅ Thêm login event handler cho webview để xử lý proxy authentication
 * 5. ✅ Thêm check proxy expiration ở runParallelProcessing để tránh webview treo
 * 6. ✅ Track proxy auth retry count per webview để phát hiện proxy hết hạn
 * 7. ✅ Persistent cleanup tracking - xóa files bị lock khi app restart
 * 8. ✅ Windows force delete fallback - dùng 'rd /s /q' cho files cứng đầu
 */
window.WEBVIEW_CLEANUP_CONFIG = {
  // Thời gian tab mặc định "ở lại trang" (phút)
  // Dựa trên: time_o_lai_trang trong google_click()
  defaultTabLifetimeMinutes: 10,
  
  // Buffer time để chắc chắn tab đã đóng + cleanup hoàn tất (phút)
  // Bao gồm: fnRemoveTab delay (10s) + fnCompleteWebviewCleanup (5s) + OS release handles (~10-30s)
  bufferMinutes: 2,
  
  /**
   * Tính toán thời gian tối đa từ khi folder được tạo/modified → có thể xóa an toàn
   * @param {number} customTabLifetimeMinutes - Thời gian "ở lại trang" custom (nếu có)
   * @returns {number} Milliseconds
   */
  getMaxAgeMs: function(customTabLifetimeMinutes) {
    const baseMinutes = customTabLifetimeMinutes || this.defaultTabLifetimeMinutes;
    const totalMinutes = baseMinutes + this.bufferMinutes;
    console.log(`[WEBVIEW_CLEANUP_CONFIG] Max age: ${totalMinutes} phút (tab lifetime: ${baseMinutes} + buffer: ${this.bufferMinutes})`);
    return totalMinutes * 60 * 1000;
  }
};

// Normalize NW.js user data root (ensure it includes Default once).
window.getWebviewUserDataRoot = function() {
  if (!window.hasOwnProperty('process')) return null;
  try {
    const gui = require('nw.gui');
    const Path = require('path');
    const dataPath = gui.App.dataPath || '';
    return Path.basename(dataPath) === 'Default' ? dataPath : Path.join(dataPath, 'Default');
  } catch (err) {
    console.warn('[getWebviewUserDataRoot] Failed to resolve user data root:', err.message);
    return null;
  }
};

/**
 * DIAGNOSTIC: List orphaned partition folders (exist on disk but not tracked)
 */
window.listOrphanedPartitions = function() {
  if (!window.hasOwnProperty('process')) {
    console.warn('[listOrphanedPartitions] Node.js not available');
    return { orphaned: [], active: [], error: 'Node.js not available' };
  }
  
  try {
    const fs = require('fs');
    const Path = require('path');
    const userDataRoot = window.getWebviewUserDataRoot();
    
    if (!userDataRoot) {
      return { orphaned: [], active: [], error: 'Cannot determine userDataRoot' };
    }
    
    console.log(`\n╔══════════════════════════════════════════════════════════════╗`);
    console.log(`║  ORPHANED PARTITIONS DIAGNOSTIC`);
    console.log(`╚══════════════════════════════════════════════════════════════╝`);
    console.log(`📂 User Data Root: ${userDataRoot}\n`);
    
    // Get active partition IDs from DOM webviews
    const activeWebviews = document.querySelectorAll('webview[id^="U_"]');
    const activePartitions = new Set();
    
    console.log(`📊 Active Webviews in DOM: ${activeWebviews.length}`);
    activeWebviews.forEach((wv, idx) => {
      const partition = wv.getAttribute('partition') || wv.dataset.partitionId;
      if (partition) {
        activePartitions.add(partition);
        console.log(`  ${idx + 1}. ${wv.id}: ${partition}`);
      }
    });
    
    // Scan partition folders on disk
    const partitionsDir = Path.join(userDataRoot, 'Partitions');
    const orphanedPartitions = [];
    const allDiskPartitions = [];
    
    if (fs.existsSync(partitionsDir)) {
      console.log(`\n📁 Scanning disk: ${partitionsDir}`);
      const entries = fs.readdirSync(partitionsDir);
      const persistFolders = entries.filter(e => e.startsWith('persist:'));
      
      console.log(`   Found ${persistFolders.length} persist:* folders on disk\n`);
      
      persistFolders.forEach((folder) => {
        ...
        allDiskPartitions.push({ name: folder, path: fullPath, size });
        
        if (!activePartitions.has(folder)) {
          orphanedPartitions.push({ name: folder, path: fullPath, size });
        }
      });
      
      // Print results
      if (orphanedPartitions.length > 0) {
        console.log(`⚠️  ORPHANED PARTITIONS (not in active webviews): ${orphanedPartitions.length}`);
        let totalOrphanedSize = 0;
        orphanedPartitions.forEach((p, idx) => {
          const sizeMB = (p.size / 1024 / 1024).toFixed(2);
          totalOrphanedSize += p.size;
          console.log(`   ${idx + 1}. ${p.name} (${sizeMB} MB)`);
        });
        console.log(`   📦 Total orphaned size: ${(totalOrphanedSize / 1024 / 1024).toFixed(2)} MB\n`);
      } else {
        console.log(`✅ No orphaned partitions found!\n`);
      }
      
      // Total disk usage
      let totalDiskSize = 0;
      allDiskPartitions.forEach(p => totalDiskSize += p.size);
      console.log(`💾 Total partition disk usage: ${(totalDiskSize / 1024 / 1024).toFixed(2)} MB`);
      console.log(`   - Active: ${activePartitions.size} partitions`);
      console.log(`   - Orphaned: ${orphanedPartitions.length} partitions\n`);
      
    } else {
      console.log(`⚠️  Partitions directory not found: ${partitionsDir}\n`);
    }
    
    return {
      userDataRoot,
      activePartitions: Array.from(activePartitions),
      orphanedPartitions: orphanedPartitions.map(p => ({ name: p.name, size: p.size })),
      allDiskPartitions: allDiskPartitions.map(p => ({ name: p.name, size: p.size }))
    };
    
  } catch (err) {
    console.error(`❌ Error listing orphaned partitions:`, err.message);
    return { orphaned: [], active: [], error: err.message };
  }
};

/**
 * XÓA TRIỆT ĐỂ TẤT CẢ DỮ LIỆU CỦA MỘT PARTITION WEBVIEW
 * ⚠️ CHỈ XÓA PARTITION CỦA WEBVIEW (có prefix persist:)
 * ⚠️ KHÔNG XÓA DỮ LIỆU NW.JS APP CHÍNH
 * 
 * @param {string} partitionId - ID của partition (VD: 'persist:123456789')
 * @param {number} customMaxAgeMinutes - Thời gian tối đa để xem folder là "cũ" (phút). Mặc định: lấy từ config
 */
window.deletePartitionFilesCompletely = function(partitionId, customMaxAgeMinutes) {
  if (!window.hasOwnProperty('process')) {
    console.warn('[deletePartitionFilesCompletely] Node.js không khả dụng');
    return;
  }
  
  // Track cleanup invocations
  if (!window.__deletePartitionCallLog) {
    window.__deletePartitionCallLog = [];
  }
  const callTimestamp = new Date().toISOString();
  window.__deletePartitionCallLog.push({ partitionId, timestamp: callTimestamp, customMaxAgeMinutes });
  
  console.log(`\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  console.log(`🗑️  [deletePartitionFilesCompletely] CALLED`);
  console.log(`   Partition: ${partitionId}`);
  console.log(`   Time: ${callTimestamp}`);
  console.log(`   Max Age: ${customMaxAgeMinutes || 'default'} minutes`);
  console.log(`   Call #${window.__deletePartitionCallLog.length}`);
  console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`);
  ...
  try {
    ...
    const userDataRoot = (typeof window.getWebviewUserDataRoot === 'function'
      ? window.getWebviewUserDataRoot()
      : (Path.basename(userDataPath) === 'Default' ? userDataPath : Path.join(userDataPath, 'Default'))
    );
    
    console.log(`   📂 userDataRoot: ${userDataRoot}`);
    
    // ⚠️ SAFEGUARD 1: partitionId PHẢI bắt đầu bằng 'persist:' (của webview)
    if (!partitionId || !partitionId.startsWith('persist:')) {
      console.error('[deletePartitionFilesCompletely] ❌ CẢNH BÁO: partitionId không hợp lệ (phải bắt đầu bằng persist:):', partitionId);
      return;
    }
    
    // ⚠️ SAFEGUARD 2: partitionId PHẢI có độ dài hợp lý (> 10 ký tự)
    if (partitionId.length < 10) {
      console.error('[deletePartitionFilesCompletely] ❌ CẢNH BÁO: partitionId quá ngắn:', partitionId);
      return;
    }
    
    console.log('[deletePartitionFilesCompletely] 🗑️ Bắt đầu xóa TOÀN BỘ dữ liệu partition:', partitionId);
    console.log('[deletePartitionFilesCompletely] 📁 User Data Path:', userDataPath);
    console.log('[deletePartitionFilesCompletely] 📁 User Data Root:', userDataRoot);
    
    // Danh sách các thư mục chứa dữ liệu của partition
    // ⚠️ CHỈ XÓA CÁC THÚ MỤC CON CÓ CHỨA partitionId, KHÔNG XÓA THÚ MỤC CHA
    // Note: userDataPath thường đã là ".../User Data/Default", không cần thêm 'Default' nữa
    const foldersToClean = [
      { path: Path.join('Partitions', partitionId), name: 'Partitions' },
      { path: Path.join('Local Storage', partitionId), name: 'Local Storage' },
      { path: Path.join('Session Storage', partitionId), name: 'Session Storage' },
      { path: Path.join('IndexedDB', partitionId), name: 'IndexedDB' },
      { path: Path.join('databases', partitionId), name: 'databases' },
      { path: Path.join('Cache', partitionId), name: 'Cache' },
      { path: Path.join('Code Cache', partitionId), name: 'Code Cache' },
      { path: Path.join('GPUCache', partitionId), name: 'GPUCache' }
    ];
    ...
    foldersToClean.forEach(folder => {
      const fullPath = Path.join(userDataRoot, folder.path);
      
      // ⚠️ SAFEGUARD 3: Đường dẫn PHẢI chứa partitionId
      if (!fullPath.includes(partitionId)) {
        console.error(`[deletePartitionFilesCompletely] ❌ CẢNH BÁO: Đường dẫn không chứa partitionId, BỎ QUA: ${fullPath}`);
        skippedCount++;
        return;
      }
      
      // ⚠️ SAFEGUARD 4: Không xóa thư mục Default root
      if (fullPath.endsWith('Default') || fullPath.endsWith('Default/') || fullPath.endsWith('Default\\')) {
        console.error(`[deletePartitionFilesCompletely] ❌ CẢNH BÁO: Cố gắng xóa thư mục Default root, BỎ QUA: ${fullPath}`);
        skippedCount++;
        return;
      }
      
      if (fs.existsSync(fullPath)) {
        console.log(`[deletePartitionFilesCompletely] 🗑️ Xóa ${folder.name}: ${fullPath}`);
        try {
          deleteFolderRecursive(fullPath);
          deletedCount++;
        } catch (err) {
          console.error(`[deletePartitionFilesCompletely] ❌ Lỗi xóa ${folder.name}:`, err.message);
        }
      } else {
        console.log(`[deletePartitionFilesCompletely] ℹ️ ${folder.name} không tồn tại: ${fullPath}`);
      }
    });
    
    // ⚠️ XỬ LÝ ĐẶC BIỆT: Storage/ext có cấu trúc phức tạp
    // Cấu trúc: Storage/ext/<extension_id>/<partition_hash>/...
    // VD: Storage/ext/lihaagbedhkbdcdlgcjadhooefgmpgog/0C6F48C7B2E8/Network/Cookies
    // Partition hash (0C6F48C7B2E8) không map từ partitionId (persist:1771519909763)
    // → Xóa các folders "cũ hơn" theo thời gian user cài đặt (time_o_lai_trang)
    // 
    // 🔥 QUAN TRỌNG: Extension process cần NHIỀU thời gian hơn để terminate
    // → Delay cleanup Storage/ext thêm 15-30s so với partition folders thông thường
    console.log('[deletePartitionFilesCompletely] 🔍 Scanning Storage/ext for old partition folders...');
    console.log('[deletePartitionFilesCompletely] ⚠️ Storage/ext cleanup will be DELAYED by 20s to allow extension processes to terminate');
    
    // Schedule Storage/ext cleanup sau 20 giây
    setTimeout(() => {
      try {
      const storageExtPath = Path.join(userDataRoot, 'Storage', 'ext');
      if (fs.existsSync(storageExtPath)) {
        const extensionFolders = fs.readdirSync(storageExtPath);
        const now = Date.now();
        
        // Tính maxAge từ config (có thể custom)
        const maxAgeMs = window.WEBVIEW_CLEANUP_CONFIG.getMaxAgeMs(customMaxAgeMinutes);
        const maxAgeMinutes = Math.round(maxAgeMs / 60000);
        console.log(`[deletePartitionFilesCompletely] ⏱️ Max age for deletion: ${maxAgeMinutes} phút`);
        
        // ⚠️ SAFEGUARD: Check có webview nào đang ACTIVE không
        // Chỉ xóa folders CŨ HƠN webview cũ nhất đang chạy + buffer
        const activeWebviews = Array.from(window.__webviewStorageTracker.activeWebviews.values());
        let safeMinFolderAgeMs = maxAgeMs; // Mặc định
        
        if (activeWebviews.length > 0) {
          // Tìm webview CŨ NHẤT đang active
          const oldestActiveCreatedAt = Math.min(...activeWebviews.map(v => v.createdAt));
          const oldestActiveAgeMs = now - oldestActiveCreatedAt;
          const oldestActiveAgeMinutes = Math.round(oldestActiveAgeMs / 60000);
          
          // CHỈ xóa folders cũ hơn (webview cũ nhất + 5 phút buffer)
          // Điều này đảm bảo KHÔNG BAO GIỜ xóa folders của webviews đang chạy
          const bufferMs = 5 * 60 * 1000; // 5 phút buffer
          safeMinFolderAgeMs = Math.max(maxAgeMs, oldestActiveAgeMs + bufferMs);
          const safeMinMinutes = Math.round(safeMinFolderAgeMs / 60000);
          
          console.log(`[deletePartitionFilesCompletely] 🛡️ SAFEGUARD: ${activeWebviews.length} webview(s) đang active`);
          console.log(`[deletePartitionFilesCompletely] 🛡️ Webview cũ nhất: ${oldestActiveAgeMinutes} phút old → CHỈ xóa folders >${safeMinMinutes} phút`);
        }
        
        extensionFolders.forEach(extId => {
          const extPath = Path.join(storageExtPath, extId);
          try {
            if (fs.statSync(extPath).isDirectory()) {
              const partitionFolders = fs.readdirSync(extPath);
              partitionFolders.forEach(partitionHash => {
                const partitionPath = Path.join(extPath, partitionHash);
                try {
                  ...
                  if (stats.isDirectory()) {
                    ...
                    // Chỉ xóa folder nếu:
                    // 1. Folder > maxAge (tab đã đóng lâu)
                    // 2. VÀ folder > (webview cũ nhất active + buffer) - KHÔNG xóa folders của tabs đang chạy
                    if (ageMs > safeMinFolderAgeMs) {
                      console.log(`[deletePartitionFilesCompletely] 🗑️ Storage/ext - Xóa partition folder cũ (${ageMinutes} phút old): ${partitionPath}`);
                      
                      // Try aggressive cleanup for Storage/ext (extension storage)
                      // Extension processes có thể lock files lâu hơn webview
                      let deleted = false;
                      
                      // Method 1: Normal delete
                      try {
                        deleteFolderRecursive(partitionPath, 0, 15); // Reduce retries for ext folders
                        deletedCount++;
                        deleted = true;
                      } catch (delErr) {
                        console.warn(`[deletePartitionFilesCompletely] ⚠️ Method 1 failed: ${delErr.message}`);
                      }
                      
                      // Method 2: Windows force delete fallback
                      if (!deleted && process.platform === 'win32') {
                        try {
                          const childProcess = require('child_process');
                          childProcess.execSync(`rd /s /q "${partitionPath}"`, { timeout: 10000 });
                          console.log(`[deletePartitionFilesCompletely] ✅ Force deleted with rd command: ${partitionPath}`);
                          deletedCount++;
                          deleted = true;
                        } catch (cmdErr) {
                          console.warn(`[deletePartitionFilesCompletely] ⚠️ Method 2 (rd) failed: ${cmdErr.message}`);
                        }
                      }
                      
                      // Method 3: Mark for deferred cleanup
                      if (!deleted) {
                        console.warn(`[deletePartitionFilesCompletely] 📝 All methods failed - marking for cleanup on restart: ${partitionPath}`);
                        if (typeof window.markForDeferredCleanup === 'function') {
                          window.markForDeferredCleanup(partitionPath);
                        }
                        skippedCount++;
                      }
                    } else {
                      const reason = ageMs <= maxAgeMs 
                        ? `còn mới (${ageMinutes}m < ${maxAgeMinutes}m)` 
                        : `có thể của webview đang active (${ageMinutes}m < ${Math.round(safeMinFolderAgeMs/60000)}m safe threshold)`;
                      console.log(`[deletePartitionFilesCompletely] ⏳ Storage/ext - Bỏ qua folder ${reason}: ${partitionHash}`);
                    }
                  }
                } catch (statErr) {
                  // Skip nếu không thể access (có thể đang bị lock)
                  console.warn(`[deletePartitionFilesCompletely] ⚠️ Cannot access partition folder (may be locked): ${partitionPath}`, statErr.message);
                }
              });
            }
          } catch (extErr) {
            // Skip if cannot access extension folder
          }
        });
      } else {
        console.log('[deletePartitionFilesCompletely/Storage.ext] ℹ️ Storage/ext không tồn tại');
      }
    } catch (err) {
      console.warn('[deletePartitionFilesCompletely/Storage.ext] ⚠️ Không thể scan Storage/ext:', err.message);
    }
    }, 20000); // Delay 20s cho extension processes terminate
    
    console.log(`[deletePartitionFilesCompletely] ✅ Hoàn tất! Đã xóa: ${deletedCount}, Bỏ qua: ${skippedCount}\n`);
    
  } catch (err) {
    console.error('[deletePartitionFilesCompletely] ❌ Lỗi nghiêm trọng:', err.message, err.stack);
  }
};

/**
 * DIAGNOSTIC: View cleanup call history
 */
window.getCleanupCallLog = function() {
  if (!window.__deletePartitionCallLog || window.__deletePartitionCallLog.length === 0) {
    console.log('📋 No cleanup calls logged yet.');
    return [];
  }
  
  console.log(`\n╔══════════════════════════════════════════════════════════════╗`);
  console.log(`║  CLEANUP CALL LOG (${window.__deletePartitionCallLog.length} total calls)`);
  console.log(`╚══════════════════════════════════════════════════════════════╝\n`);
  
  window.__deletePartitionCallLog.forEach((call, idx) => {
    console.log(`${idx + 1}. ${call.timestamp}`);
    console.log(`   Partition: ${call.partitionId}`);
    console.log(`   Max Age: ${call.customMaxAgeMinutes || 'default'} minutes\n`);
  });
  
  return window.__deletePartitionCallLog;
};
...
// ⚠️ CHỈ XÓA PARTITION CỦA WEBVIEW (prefix persist:), KHÔNG XÓA DỮ LIỆU APP CHÍNH
/**
 * AGGRESSIVE CLEANUP OF ORPHANED FOLDERS
 * Cleans up old partition folders that:
 * 1. Are NOT currently in use by any active webview
 * 2. Have creation time older than max webview lifetime
 * 3. Start with 'persist:' (definitely from webview)
 * 
 * Call this after closing all webviews
 */
window.cleanupOrphanedPartitionFolders = function() {
  if (!window.hasOwnProperty("process")) return { cleaned: 0, error: null };
  ...
  try {
    ...
    console.log(`\n[OrphanedCleanup] 🧹 Scanning for orphaned partition folders...`);
    console.log(`   User data root: ${userDataRoot}`);
    
    // Get list of active webview partition IDs
    const activeWebviews = document.querySelectorAll('webview[id^="U_"]');
    const activePartitions = new Set();
    activeWebviews.forEach(wv => {
      const partition = wv.getAttribute('partition') || wv.dataset.partitionId;
      if (partition) activePartitions.add(partition);
    });
    
    console.log(`   Active webviews: ${activePartitions.size}`);
    if (activePartitions.size === 0) {
      console.log(`   INFO: All webviews closed - will clean old folders`);
    } else {
      console.log(`   WARNING: Still have active webviews, skipping cleanup`);
      return { cleaned: 0, reason: 'active_webviews_present' };
    }
    
    // Max webview lifetime (usually ~10 minutes default)
    const WEBVIEW_MAX_LIFETIME_MS = (window.__webviewStorageTracker?.defaultTabLifetimeMinutes || 10) * 60 * 1000;
    const NOW = Date.now();
    let cleanedCount = 0;
    
    // Platform-specific time check (different file systems handle mtime differently)
    const isOldEnough = (mtime) => (NOW - mtime) > WEBVIEW_MAX_LIFETIME_MS;
    
    // Check Partitions folder
    const partitionsPath = Path.join(userDataRoot, 'Partitions');
    if (fs.existsSync(partitionsPath)) {
      console.log(`\n   📁 Checking: Partitions/`);
      const folders = fs.readdirSync(partitionsPath);
      
      folders.forEach(folder => {
        // Only delete folders starting with 'persist:' (definitely webview)
        if (!folder.startsWith('persist:')) {
          return;  // Skip non-webview folders (like _crx_)
        }
        
        const folderPath = Path.join(partitionsPath, folder);
        
        try {
          const stat = fs.statSync(folderPath);
          const ctime = stat.birthtimeMs || stat.ctimeMs || stat.mtime.getTime();  // Creation/change time
          const ageMs = NOW - ctime;
          const ageMinutes = (ageMs / 60000).toFixed(1);
          
          if (isOldEnough(ctime) && !activePartitions.has(folder)) {
            console.log(`      🗑️  Deleting old partition (${ageMinutes}min old): ${folder}`);
            deleteFolderSimple(folderPath);
            cleanedCount++;
          } else if (activePartitions.has(folder)) {
            console.log(`      ⏭️  Skipping active partition: ${folder}`);
          } else {
            console.log(`      ⏭️  Keeping recent partition (${ageMinutes}min old): ${folder}`);
          }
        } catch (err) {
          console.warn(`      ⚠️  Cannot stat ${folder}:`, err.message);
        }
      });
    }
    
    // Check other storage locations
    const otherLocations = ['Local Storage', 'Cache', 'IndexedDB', 'Session Storage'];
    otherLocations.forEach(location => {
      const locPath = Path.join(userDataRoot, location);
      if (fs.existsSync(locPath)) {
        console.log(`\n   📁 Checking: ${location}/`);
        const folders = fs.readdirSync(locPath);
        
        folders.forEach(folder => {
          if (!folder.startsWith('persist:')) return;
          
          const folderPath = Path.join(locPath, folder);
          try {
            const stat = fs.statSync(folderPath);
            const ctime = stat.birthtimeMs || stat.ctimeMs || stat.mtime.getTime();
            const ageMs = NOW - ctime;
            const ageMinutes = (ageMs / 60000).toFixed(1);
            
            if (isOldEnough(ctime) && !activePartitions.has(folder)) {
              console.log(`      🗑️  Deleting old ${location}/${folder} (${ageMinutes}min old)`);
              deleteFolderSimple(folderPath);
              cleanedCount++;
            }
          } catch (err) {}
    ...
    console.log(`\n[OrphanedCleanup] ✅ Cleanup complete - cleaned ${cleanedCount} old folders\n`);
    return { cleaned: cleanedCount, error: null };
    
  } catch (err) {
    console.error(`[OrphanedCleanup] ❌ Error:`, err.message);
...
window.cleanupAllPartitions = function(options = {}) {
  ...
  const { 
    cleanPartitions = true,      // Xóa Partitions/persist:*
    cleanStorageExt = true,       // Xóa Storage/ext/persist:*
    cleanLocalStorage = false,    // KHÔNG xóa Local Storage (có thể chứa data của app)
    cleanCache = false,           // KHÔNG xóa Cache (có thể chứa cache của app)
    cleanIndexedDB = false        // KHÔNG xóa IndexedDB (có thể chứa data của app)
  } = options;
  ...
  try {
    ...
    console.log('[CLEANUP-PARTITIONS] 🧹 Bắt đầu cleanup các partition cũ của WEBVIEW...');
    
    // 1. Xóa CHỈ các partitions CỦA WEBVIEW (prefix persist:)
    if (cleanPartitions) {
      const partitionsPath = Path.join(userDataRoot, 'Partitions');
      if (fs.existsSync(partitionsPath)) {
        console.log('[CLEANUP-PARTITIONS] 🗑️ Quét thư mục Partitions:', partitionsPath);
        const partitionFolders = fs.readdirSync(partitionsPath);
        let cleaned = 0;
        
        partitionFolders.forEach(folder => {
          // CHỈ xóa partition có prefix "persist:" (của webview)
          if (folder.startsWith('persist:') || folder.startsWith('persist_')) {
            const folderPath = Path.join(partitionsPath, folder);
            console.log(`[CLEANUP-PARTITIONS] 🗑️ Xóa partition webview: ${folder}`);
            try {
              deleteFolderRecursive(folderPath);
              cleaned++;
            } catch (err) {
              console.error(`[CLEANUP-PARTITIONS] ❌ Lỗi xóa ${folder}:`, err.message);
            }
          } else {
            console.log(`[CLEANUP-PARTITIONS] ⏭️ Bỏ qua partition không phải webview: ${folder}`);
          }
        });
        
        console.log(`[CLEANUP-PARTITIONS] ✅ Đã xóa ${cleaned} webview partitions`);
      }
    }
    
    // 2. Xóa Storage/ext CHỈ CỦA WEBVIEW (prefix persist:)
    if (cleanStorageExt) {
      const storageExtPath = Path.join(userDataRoot, 'Storage', 'ext');
      if (fs.existsSync(storageExtPath)) {
        console.log('[CLEANUP-PARTITIONS] 🗑️ Quét thư mục Storage/ext:', storageExtPath);
        const storageFolders = fs.readdirSync(storageExtPath);
        let cleaned = 0;
        
        storageFolders.forEach(folder => {
          // CHỈ xóa storage có prefix "persist:" (của webview)
          if (folder.startsWith('persist:') || folder.startsWith('persist_')) {
            const folderPath = Path.join(storageExtPath, folder);
            console.log(`[CLEANUP-PARTITIONS] 🗑️ Xóa storage webview: ${folder}`);
            try {
              deleteFolderRecursive(folderPath);
              cleaned++;
            } catch (err) {
            ...
            console.log(`[CLEANUP-PARTITIONS] ⏭️ Bỏ qua storage không phải webview: ${folder}`);
          }
        });
        
        console.log(`[CLEANUP-PARTITIONS] ✅ Đã xóa ${cleaned} webview storages`);
      }
    }
    
    // 3. Xóa Local Storage của WEBVIEW (tùy chọn, mặc định KHÔNG)
    if (cleanLocalStorage) {
      const localStoragePath = Path.join(userDataRoot, 'Local Storage');
      if (fs.existsSync(localStoragePath)) {
        console.log('[CLEANUP-PARTITIONS] 🗑️ Quét Local Storage:', localStoragePath);
        const folders = fs.readdirSync(localStoragePath);
        let cleaned = 0;
        
        folders.forEach(folder => {
          if (folder.startsWith('persist:') || folder.startsWith('persist_')) {
            const folderPath = Path.join(localStoragePath, folder);
            console.log(`[CLEANUP-PARTITIONS] 🗑️ Xóa local storage: ${folder}`);
            try {
              deleteFolderRecursive(folderPath);
              cleaned++;
            } catch (err) {
        ...
        console.log(`[CLEANUP-PARTITIONS] ✅ Đã xóa ${cleaned} local storages`);
      }
    }
    
    // 4. Xóa IndexedDB của WEBVIEW (tùy chọn, mặc định KHÔNG)
    if (cleanIndexedDB) {
      const indexedDBPath = Path.join(userDataRoot, 'IndexedDB');
      if (fs.existsSync(indexedDBPath)) {
        console.log('[CLEANUP-PARTITIONS] 🗑️ Quét IndexedDB:', indexedDBPath);
        const folders = fs.readdirSync(indexedDBPath);
        let cleaned = 0;
        
        folders.forEach(folder => {
          if (folder.startsWith('persist:') || folder.startsWith('persist_')) {
            const folderPath = Path.join(indexedDBPath, folder);
            console.log(`[CLEANUP-PARTITIONS] 🗑️ Xóa indexedDB: ${folder}`);
            try {
              deleteFolderRecursive(folderPath);
              cleaned++;
            } catch (err) {
        ...
        console.log(`[CLEANUP-PARTITIONS] ✅ Đã xóa ${cleaned} indexedDBs`);
      }
    }
    
    // ⚠️ KHÔNG xóa Cache mặc định vì có thể ảnh hưởng đến app chính
    if (cleanCache) {
      console.warn('[CLEANUP-PARTITIONS] ⚠️ cleanCache=true: Sẽ xóa cache, có thể ảnh hưởng app!');
      const cachePaths = [
        Path.join(userDataRoot, 'Cache'),
        Path.join(userDataRoot, 'Code Cache'),
        Path.join(userDataRoot, 'GPUCache')
      ];
      
      cachePaths.forEach(cachePath => {
        if (fs.existsSync(cachePath)) {
          console.log('[CLEANUP-PARTITIONS] 🗑️ Xóa cache:', cachePath);
          try {
            deleteFolderRecursive(cachePath);
          } catch (err) {
            console.error('[CLEANUP-PARTITIONS] ❌ Lỗi xóa cache:', err.message);
          }
        }
      });
    }
    
    console.log('[CLEANUP-PARTITIONS] ✅ Hoàn tất cleanup partitions của webview!');
  } catch (err) {
    console.error('[CLEANUP-PARTITIONS] ❌ Lỗi:', err.message, err.stack);
  }
};

/**
 * MONITOR DUNG LƯỢNG STORAGE CỦA WEBVIEW PARTITIONS
 * Giúp theo dõi và phát hiện partition nào đang chiếm nhiều dung lượng
 */
window.getPartitionStorageInfo = function() {
  if (!window.hasOwnProperty('process')) {
    console.warn('[getPartitionStorageInfo] Node.js không khả dụng');
    return null;
  }
  ...
  try {
      ...
      try {
        ...
        files.forEach(file => {
          ...
          try {
            ...
            console.warn('[getFolderSize] Cannot stat file:', filePath, e.message);
          }
        });
      } catch (e) {
        console.error('[getFolderSize] Cannot read directory:', dirPath, e.message);
      }
    ...
    if (fs.existsSync(partitionsPath)) {
      ...
      folders.forEach(folder => {
        if (folder.startsWith('persist:') || folder.startsWith('persist_')) {
          ...
          storageInfo.totalPartitions++;
        }
      });
    }
    
    // Scan Storage/ext folder
    const storageExtPath = Path.join(userDataRoot, 'Storage', 'ext');
    if (fs.existsSync(storageExtPath)) {
      const folders = fs.readdirSync(storageExtPath);
      folders.forEach(folder => {
        if (folder.startsWith('persist:') || folder.startsWith('persist_')) {
          const folderPath = Path.join(storageExtPath, folder);
          const size = getFolderSize(folderPath);
          
          // Tìm partition tương ứng để cộng dồn size
          const existingPartition = storageInfo.partitions.find(p => p.id === folder);
          if (existingPartition) {
            existingPartition.size += size;
            existingPartition.sizeFormatted = formatSize(existingPartition.size);
          } else {
            storageInfo.partitions.push({
              id: folder,
              type: 'Storage/ext',
    ...
    storageInfo.totalSizeFormatted = formatSize(storageInfo.totalSize);
    
    // Sort by size descending
    storageInfo.partitions.sort((a, b) => b.size - a.size);
    
    console.log('[getPartitionStorageInfo] 📊 Storage Info:', storageInfo);
    console.log(`[getPartitionStorageInfo] 💾 Tổng dung lượng: ${storageInfo.totalSizeFormatted} (${storageInfo.totalPartitions} partitions)`);
    
    return storageInfo;
  } catch (err) {
    console.error('[getPartitionStorageInfo] ❌ Lỗi:', err.message);
    return null;
  }
};
...
window.__proxyResetInProgress = false;
window.isProxyExpired = function() {
  return window.__isProxyActive && window.__proxyActivatedTime > 0 &&
    (Date.now() - window.__proxyActivatedTime) >= window.PROXY_MAX_LIFETIME;
};

/**
 * PROXY HEALTH CHECK - Kiểm tra proxy còn hoạt động không
 * Dùng TCP ping thay vì HTTP request để tránh trigger proxy authentication dialog
 */
window.checkProxyHealth = async function() {
  // Không check nếu proxy chưa active hoặc đang check
  if (!window.__isProxyActive || window.__proxyHealthCheckInProgress) {
    return { healthy: true, reason: 'skip' };
  }
  
  // Không có thông tin proxy host/port
  if (!window.__proxyHost || !window.__proxyPort) {
    console.warn('⚠️ [ProxyHealthCheck] Không có proxy host/port để check');
    return { healthy: true, reason: 'no-proxy-info' };
  }
  
  // Throttle: chỉ check mỗi PROXY_HEALTH_CHECK_INTERVAL
  const now = Date.now();
  if (now - window.__lastProxyHealthCheck < window.PROXY_HEALTH_CHECK_INTERVAL) {
    return { healthy: true, reason: 'throttled' };
  }
  
  window.__proxyHealthCheckInProgress = true;
  window.__lastProxyHealthCheck = now;
  
  console.log(`🏥 [ProxyHealthCheck] Ping TCP ${window.__proxyHost}:${window.__proxyPort}...`);
  
  try {
    // Dùng Node.js net module để ping TCP (có sẵn trong NW.js)
    if (typeof require === 'undefined' || !window.hasOwnProperty('process')) {
      console.warn('⚠️ [ProxyHealthCheck] Node.js net module không có sẵn, fallback check');
      window.__proxyHealthCheckInProgress = false;
      return { healthy: true, reason: 'no-nodejs' };
    }
    ...
    const result = await new Promise((resolve) => {
      ...
      // Error - proxy unreachable or down
      socket.on('error', (err) => {
        clearTimeout(timer);
        done(false, 'error: ' + err.code);
      });
      
      // Try connect
      socket.connect({
        host: window.__proxyHost,
        port: window.__proxyPort
      });
    });
    
    if (result.healthy) {
      console.log(`✅ [ProxyHealthCheck] Proxy TCP ping OK - ${window.__proxyHost}:${window.__proxyPort} đang sống`);
      window.__proxyHealthCheckFailures = 0; // Reset failure count
      window.__proxyHealthCheckInProgress = false;
      return result;
    } else {
      window.__proxyHealthCheckFailures++;
      console.warn(`⚠️ [ProxyHealthCheck] Proxy TCP ping failed (${result.reason}). Failures: ${window.__proxyHealthCheckFailures}/${window.PROXY_HEALTH_CHECK_MAX_FAILURES}`);
      
      // Nếu fail quá nhiều lần, đánh dấu cần reset
      if (window.__proxyHealthCheckFailures >= window.PROXY_HEALTH_CHECK_MAX_FAILURES) {
        console.error(`🚨 [ProxyHealthCheck] Proxy đã fail ${window.__proxyHealthCheckFailures} lần liên tiếp - CẦN RESET!`);
        window.__proxyNeedsReset = true;
        window.__proxyHealthCheckFailures = 0; // Reset counter
      }
    ...
    console.error('❌ [ProxyHealthCheck] Lỗi khi check:', err);
    window.__proxyHealthCheckFailures++;
    
    if (window.__proxyHealthCheckFailures >= window.PROXY_HEALTH_CHECK_MAX_FAILURES) {
      console.error(`🚨 [ProxyHealthCheck] Proxy check error ${window.__proxyHealthCheckFailures} lần - CẦN RESET!`);
      window.__proxyNeedsReset = true;
      window.__proxyHealthCheckFailures = 0;
    }
    
    window.__proxyHealthCheckInProgress = false;
    return { healthy: false, reason: 'exception', error: err.message };
  }
};

// Guard: kiểm tra proxy trước khi tạo webview mới
window.ensureProxyFreshBeforeWebview = function(trigger = 'unknown') {
  if (window.__proxyResetInProgress) {
    console.warn(`[ProxyGuard] Reset đang chạy, skip tạo webview (${trigger})`);
    return false;
  }

  const expired = window.isProxyExpired();
  if (window.__proxyNeedsReset || expired) {
    const reason = window.__proxyNeedsReset ? 'proxy_needs_reset' : 'proxy_expired';
    console.warn(`[ProxyGuard] ${reason} trước khi mở webview (${trigger}) → reset proxy`);
    window.__proxyResetInProgress = true;
    window.__proxyNeedsReset = true;

    try {
      if (typeof closeAllTabsAndCleanup === 'function') {
        closeAllTabsAndCleanup(`Proxy ${reason}`);
      }
    } catch (e) {
      console.error('[ProxyGuard] Error closing tabs:', e);
    }

    setTimeout(() => {
      try {
        if (typeof fnResetIP === 'function') {
          fnResetIP(true);
        }
      } catch (e) {
        window.__proxyResetInProgress = false;
...
window.__proxyChangePreparationInProgress = false;

window.waitForAllTabsClosedAndCleaned = function(options = {}) {
  ...
  return new Promise((resolve) => {
    ...
    const check = () => {
      const activeWebviews = document.querySelectorAll('[id^="U_"]:not([id*="tabSetting"])').length;
      const trackerCount = window.__webviewStorageTracker?.activeWebviews?.size || 0;
      const queueIdle = window.__cleanupQueue && typeof window.__cleanupQueue.isIdle === 'function'
        ? window.__cleanupQueue.isIdle()
        : true;

      if (activeWebviews === 0 && trackerCount === 0 && queueIdle) {
        resolve({ success: true, elapsedMs: Date.now() - start });
        return;
      }
...
window.prepareForProxyChange = async function(reason, options = {}) {
  if (window.__proxyChangePreparationInProgress) {
    console.warn('[prepareForProxyChange] ⚠️ Đang chuẩn bị đổi proxy, bỏ qua yêu cầu mới');
    return { success: false, reason: 'already_in_progress' };
  }

  window.__proxyChangePreparationInProgress = true;
  try {
    if (typeof closeAllTabsAndCleanup === 'function') {
      closeAllTabsAndCleanup(reason || 'Proxy change');
    } else {
      console.warn('[prepareForProxyChange] ⚠️ closeAllTabsAndCleanup chưa có');
    }

    const result = await window.waitForAllTabsClosedAndCleaned(options);
    if (!result.success) {
      console.warn('[prepareForProxyChange] ⚠️ Timeout chờ tabs đóng/cleanup:', result);
    }
    return result;
  } finally {
    window.__proxyChangePreparationInProgress = false;
...
window.fnResetIP = async function (force = false) {
  ...
  webviewsToClose.forEach((tab_id, index) => {
    setTimeout(() => {
      try {
        ...
        // Khi đóng hết tabs, chạy cleanup toàn diện
        if (closedCount === webviewsToClose.length) {
          setTimeout(() => {
            ...
            // 🔥 AGGRESSIVE: Delete old orphaned partition folders
            // Only run this when ALL webviews are closed
            if (typeof window.cleanupOrphanedPartitionFolders === 'function') {
              console.log('[closeAllTabsAndCleanup] 🗑️ Cleaning orphaned partition folders...');
              const orphanResult = window.cleanupOrphanedPartitionFolders();
              console.log(`[closeAllTabsAndCleanup] ✅ Orphaned cleanup: ${orphanResult.cleaned} folders deleted`);
            }
...
// ============================================
  if (shouldCloseTabs) {
    if (typeof window.prepareForProxyChange === 'function') {
      const prepResult = await window.prepareForProxyChange(closeReason, { timeoutMs: 90000, pollMs: 500 });
      if (!prepResult.success) {
        console.warn('[fnResetIP] ⚠️ Chuẩn bị đổi proxy chưa hoàn tất:', prepResult);
      }
    } else if (typeof closeAllTabsAndCleanup === 'function') {
      closeAllTabsAndCleanup(closeReason);
    } else {
      console.warn('[fnResetIP] ⚠️ closeAllTabsAndCleanup chưa có, không thể đóng tabs');
    }
  } else {
        ...
        try {
          ...
          if ((msg.data && (msg.data['proxy'] || msg.data['https'])) && (msg.success || msg.code === 0 || msg.errorCode === 0)) {
            ...
            proxy_status_check().then(function(isProxyOn) {
              ...
              // Lưu proxy host/port để dùng cho health check
              // Format thường là: "host:port" hoặc "http://host:port"
              try {
                const proxyAddr = proxyInfo.proxy || proxyInfo.https || '';
                const match = proxyAddr.match(/(?:https?:\/\/)?([^:]+):(\d+)/);
                if (match) {
                  window.__proxyHost = match[1];
                  window.__proxyPort = parseInt(match[2], 10);
                  console.log(`🔍 Lưu proxy address cho health check: ${window.__proxyHost}:${window.__proxyPort}`);
                }
              } catch (e) {
                console.warn('⚠️ Không thể parse proxy address:', e);
              }
              ...
              proxy_change_address(proxyInfo).then(function (proxyActive) {
                ...
                if (proxyActive) {
                  ...
                  window.__proxyHealthCheckFailures = 0; // Reset health check failures
                  window.__lastProxyHealthCheck = 0; // Reset last check time
...
window.TabManager = {
  ...
  waitForSlot: function(timeout = 300000) {
      ...
      const checkInterval = setInterval(() => {
        ...
        if (this.isWaitingForBatchClose) {
          ...
          if (now - lastWarnTime > 30000) {
            ...
            console.warn(`[TabManager] ⏸️ Đang chờ batch tabs tắt, không thể tạo tab mới (${elapsedSecs}s)`);
            lastWarnTime = now;
          }
        }
        ...
        if (isProxyExpired) {
          ...
          clearInterval(checkInterval);
          reject(new Error('Timeout waiting for tab slot'));
...
const runParallelProcessing = async () => {
  ...
  // 0. CRITICAL: Kiểm tra proxy có hết hạn không trước tiên
  if (typeof window.isProxyExpired === 'function' && window.isProxyExpired()) {
    console.error('🚨 [Parallel] Proxy đã hết hạn - Dừng xử lý và reset proxy');
    window.__proxyNeedsReset = true;
    if (typeof closeAllTabsAndCleanup === 'function') {
      closeAllTabsAndCleanup('Proxy expired');
    }
    setTimeout(() => {
      if (typeof fnResetIP === 'function') {
        fnResetIP(true);
  ...
  if (typeof window.ensureProxyFreshBeforeWebview === 'function') {
    if (!window.ensureProxyFreshBeforeWebview('runParallelProcessing')) {
      return;
    }
  }
...
// Throttle reload để tránh loop vô hạn khi webview gặp lỗi liên tục.
if (!window.__webviewReloadTracker) {
  window.__webviewReloadTracker = new Map(); // id_tab -> { count, lastAttemptAt, pending }
}

window.scheduleWebviewReload = function(id_tab, reason, options = {}) {
  const {
    delayMs = 5000,
    maxRetries = 3,
    resetAfterMs = 60000
  ...
  const now = Date.now();
  const tracker = window.__webviewReloadTracker;
  const state = tracker.get(id_tab) || { count: 0, lastAttemptAt: 0, pending: false };
  ...
  if (state.count >= maxRetries) {
    console.warn(`[WebviewReload] ⛔ Max retries reached for ${id_tab} (${reason})`);
    return false;
  }
  ...
  state.pending = true;
  tracker.set(id_tab, state);

  console.log(`[WebviewReload] 🔄 Scheduling reload for ${id_tab} (${reason}) in ${delayMs}ms [${state.count}/${maxRetries}]`);

  setTimeout(() => {
    try {
      const target = document.querySelector('#' + webviewId);
      if (target) target.reload();
    } catch (err) {
      console.warn(`[WebviewReload] ⚠️ Reload failed for ${id_tab}:`, err.message);
    } finally {
      const current = tracker.get(id_tab);
      if (current) {
        current.pending = false;
        tracker.set(id_tab, current);
      }
    }
  }, delayMs);
...
window.fnCreateTab = function (id_tab, url_open, script_code, multi_tab_name, auto_close) {
  if (!isRunning)
    return;
  if (id_tab !== 'reset3G' && typeof window.ensureProxyFreshBeforeWebview === 'function') {
    if (!window.ensureProxyFreshBeforeWebview('fnCreateTab')) {
      return;
    }
  }
  try {
    ...
    // QUAN TRỌNG: Lưu partitionId để fnRemoveTab có thể xóa storage files
    webview.dataset.partitionId = 'persist:' + rand;
    ...
    webview.addEventListener("did-fail-load", (event) => {
        ...
        // QUAN TRỌNG: Xử lý lỗi proxy authentication (407)
        if (event.errorCode === -501 || event.errorDescription?.includes('407') || event.errorDescription?.includes('Proxy Authentication Required')) {
            console.error(`🚨 [Webview ${id_tab}] Proxy authentication failed - Error ${event.errorCode}: ${event.errorDescription}`);
            window.__proxyNeedsReset = true;
            window.__proxyFailureCount++;
            // Nếu nhiều webviews gặp lỗi proxy, trigger reset proxy
            if (window.__proxyFailureCount >= 2) {
                console.error('🚨 Multiple webviews failed proxy auth - Reset proxy ngay');
                if (typeof closeAllTabsAndCleanup === 'function') {
                    closeAllTabsAndCleanup('Proxy authentication failed');
                }
                if (typeof fnResetIP === 'function') {
                    setTimeout(() => fnResetIP(true), 2000);
                }
            }
        }
    });
    
    // CRITICAL FIX: Login event handler - xử lý proxy authentication
    // Khi proxy yêu cầu authentication (407), webview sẽ trigger event 'login'
    // Phải cung cấp credentials ngay để tránh webview bị treo
    webview.addEventListener('login', (event) => {
        const authInfo = event.authInfo;
        console.log(`🔐 [Webview ${id_tab}] Login event triggered - ${authInfo.isProxy ? 'PROXY' : 'HTTP'} auth required for ${authInfo.host}:${authInfo.port}`);
        
        if (authInfo.isProxy) {
            // Kiểm tra có credentials không
            if (!window.__proxyUsername || !window.__proxyPassword) {
                console.error(`❌ [Webview ${id_tab}] Proxy credentials không có - webview sẽ bị treo!`);
                window.__proxyNeedsReset = true;
                // Cancel authentication để tránh treo
                event.request.cancel();
                return;
            }
            
            // Track retry count cho webview này
            const webviewKey = `webview_${id_tab}`;
            const retryCount = (window.__proxyAuthRetryCount.get(webviewKey) || 0) + 1;
            window.__proxyAuthRetryCount.set(webviewKey, retryCount);
            
            if (retryCount > 3) {
                console.error(`❌ [Webview ${id_tab}] Proxy auth failed ${retryCount} times - credentials có thể sai hoặc proxy hết hạn`);
                window.__proxyNeedsReset = true;
                event.request.cancel();
                return;
            }
            
            console.log(`✅ [Webview ${id_tab}] Providing proxy credentials (attempt ${retryCount}/3): ${window.__proxyUsername}@***`);
            event.request.credentials = {
                username: window.__proxyUsername,
                password: window.__proxyPassword
            };
        } else {
            // HTTP auth - không phải proxy, cancel
            event.request.cancel();
        }
    });
    
    webview.addEventListener("dom-ready", function () {
        // Reset proxy auth retry count khi page load thành công
        const webviewKey = `webview_${id_tab}`;
        if (window.__proxyAuthRetryCount.has(webviewKey)) {
            window.__proxyAuthRetryCount.delete(webviewKey);
        }
        // Reset failure count khi có webview thành công
        if (window.__proxyFailureCount > 0) {
            window.__proxyFailureCount = Math.max(0, window.__proxyFailureCount - 1);
        }
    ...
    webview.addEventListener('exit', function (e) {
        if (e.reason === 'crash') {
        window.scheduleWebviewReload(id_tab, 'exit-crash', {
          delayMs: 15000,
          maxRetries: 2,
          resetAfterMs: 60000
        });
    ...
    webview.addEventListener('consolemessage', (event) => {
        ...
        if (event.message.includes("ERR_BLOCKED_BY_CLIENT") || event.message.includes("ERR_ABORTED")) {
          ...
          window.scheduleWebviewReload(id_tab, 'blocked-or-aborted', {
            delayMs: 5000,
            maxRetries: 2,
            resetAfterMs: 30000
          });
      ...
      try {
        ...
        if (message.hasOwnProperty("type")) {
          ...
          if (message.type === "title") {
            ...
            window.__proxyNeedsReset = true;
            if (isRunning && typeof window.fnResetIP === 'function') {
              try {
                window.fnResetIP();
              } catch (resetErr) {
                console.error('❌ Error calling fnResetIP:', resetErr.message);
              }
            } else {
              console.warn('⚠️ fnResetIP not available or not running');
            }
          }
        }
      } catch { }
    ...
    webview.addEventListener('loadabort',function(e) {
      try{
        window.scheduleWebviewReload(id_tab, 'loadabort', {
            delayMs: 15000,
            maxRetries: 2,
            resetAfterMs: 60000
        });
      }catch(e){
        console.error('[fnCreateTab] Lỗi:', e);
      }
    });
    if (script_code) {
    ...
    webview.addEventListener('loadstop', function (e) {
      ...
      // Xử lý Tự động đóng tab - FALLBACK nếu script không gửi close signal
      if (auto_close && auto_close > 0) {
        setTimeout(function () {
          console.log(`[auto-close] ⏰ Webview lifetime expired: automatically closing ${id_tab}`);
          if (typeof fnRemoveTab === 'function') fnRemoveTab(id_tab);
        }, auto_close + 5000); // Add buffer for script close signal
      }
    });
    ...
    // 🔍 TRACKING: Đăng ký webview vào tracker
    const partitionId = webview.getAttribute('partition');
    // Tính tabLifetimeMinutes từ auto_close (ms → phút)
    // auto_close format: (time_o_lai_trang * 60000 + 30000)
    const tabLifetimeMinutes = auto_close ? Math.ceil(auto_close / 60000) : 10;
    window.__webviewStorageTracker.registerWebview(id_tab, partitionId, url_open, tabLifetimeMinutes);
...
// ============================================
// COMPREHENSIVE WEBVIEW STORAGE CLEANUP SYSTEM
// Dọn dẹp TRIỆT ĐỂ tất cả file/folder webview tạo ra
// ============================================

/**
 * Global WebView Storage Tracker
 * Theo dõi tất cả webviews + storage của chúng
 */
window.__webviewStorageTracker = {
  activeWebviews: new Map(), // id_tab -> { partitionId, url, createdAt, size }
  cleanupLog: [], // Log tất cả cleanup operations
  totalCleaned: 0, // Tổng dung lượng đã dọn (bytes)
  
  registerWebview: function(id_tab, partitionId, url, tabLifetimeMinutes) {
    this.activeWebviews.set(id_tab, {
      partitionId: partitionId,
      url: url,
      createdAt: Date.now(),
      tabLifetimeMinutes: tabLifetimeMinutes || 10, // Default 10 phút
      size: 0
    });
    console.log(`[WebviewTracker] ✅ Registered: ${id_tab} (partition: ${partitionId}, lifetime: ${tabLifetimeMinutes || 10} phút)`);
  },
  
  unregisterWebview: function(id_tab) {
    if (this.activeWebviews.has(id_tab)) {
      this.activeWebviews.delete(id_tab);
      console.log(`[WebviewTracker] ✅ Unregistered: ${id_tab}`);
    }
  },
  
  getActivePartitions: function() {
    return Array.from(this.activeWebviews.values()).map(v => v.partitionId);
  },
  
  logCleanup: function(id_tab, partitionId, itemsRemoved, sizeFreed) {
    const logEntry = {
      ...
      sizeFreed: sizeFreed,
      freeSizeMB: (sizeFreed / 1024 / 1024).toFixed(2)
    };
    this.cleanupLog.push(logEntry);
    this.totalCleaned += sizeFreed;
    console.log(`[Cleanup Log] ${logEntry.timestamp}: ${id_tab} - ${logEntry.freeSizeMB} MB freed`);
  },
  
  printReport: function() {
    console.log(`
╔════════════════════════════════════════════════════════════════╗
║       WEBVIEW STORAGE CLEANUP REPORT                           ║
╚════════════════════════════════════════════════════════════════╝
`);
    console.log(`Total Cleaned: ${(this.totalCleaned / 1024 / 1024).toFixed(2)} MB`);
    console.log(`Operations: ${this.cleanupLog.length}`);
    this.cleanupLog.slice(-10).forEach((log, idx) => {
      console.log(`  ${idx + 1}. [${log.timestamp}] ${log.id_tab}: ${log.freeSizeMB} MB`);
    });
  }
};

// Optional hard-kill fallback for stuck webview processes.
if (typeof window.WEBVIEW_FORCE_KILL_ON_CLOSE === 'undefined') {
  window.WEBVIEW_FORCE_KILL_ON_CLOSE = true; // Đổi thành true để đảm bảo process chết
}

/**
 * Kiểm tra process có đang chạy không
 * @param {number} processId - Process ID cần kiểm tra
 * @returns {boolean} - true nếu process đang chạy
 */
window.isProcessRunning = function(processId) {
  if (!window.hasOwnProperty('process') || !processId) return false;
  if (processId === process.pid) return true; // Main process always running
  
  try {
    // Trick: process.kill(pid, 0) không kill nhưng check tồn tại
    // Nếu process tồn tại -> không throw error
    // Nếu process không tồn tại -> throw ESRCH error
    process.kill(processId, 0);
    return true; // Process đang chạy
  } catch (err) {
    if (err.code === 'ESRCH') {
      return false; // Process không tồn tại
    }
    // Lỗi khác (VD: EPERM - không có quyền) -> giả định process vẫn chạy
    return true;
  }
};

/**
 * Đợi process terminate hoàn toàn bằng cách polling
 * @param {number} processId - Process ID cần đợi
 * @param {number} timeoutMs - Timeout (ms)
 * @param {number} pollIntervalMs - Interval giữa các lần kiểm tra
 * @returns {Promise<boolean>} - true nếu process đã chết
 */
window.waitForProcessDeath = function(processId, timeoutMs = 10000, pollIntervalMs = 200) {
  return new Promise((resolve) => {
    if (!processId || !window.hasOwnProperty('process')) {
      return resolve(true); // Không có process để đợi
    }
    ...
    const checkInterval = setInterval(() => {
      const isRunning = window.isProcessRunning(processId);
      const elapsedMs = Date.now() - startTime;
      
      if (!isRunning) {
        clearInterval(checkInterval);
        console.log(`✅ [ProcessDeath] Process ${processId} đã terminate sau ${elapsedMs}ms`);
        resolve(true);
        return;
      }
      
      // After 3 seconds, if still running, force kill it
      if (elapsedMs > 3000 && !forceKillAttempted) {
        forceKillAttempted = true;
        console.warn(`⚠️ [ProcessDeath] Process still running after 3s - forcing SIGKILL on PID ${processId}`);
      ...
      if (elapsedMs >= timeoutMs) {
        clearInterval(checkInterval);
        console.error(`❌ [ProcessDeath] Process ${processId} STILL RUNNING after ${timeoutMs}ms! Will attempt cleanup with locked files.`);
        resolve(false);
      }
    }, pollIntervalMs);
  });
...
window.__forceKillWebviewProcess = function(processId, partitionId) {
  if (!window.hasOwnProperty('process') || !processId) return;
  if (processId === process.pid) return; // Never kill main process

  try {
    if (process.platform === 'win32') {
      const childProcess = require('child_process');
      // Windows: /T = kill process tree, /F = force
      childProcess.execSync(`taskkill /PID ${processId} /T /F`, { timeout: 5000 });
    } else {
      process.kill(processId, 'SIGKILL');
    }
    console.warn(`[WebviewKill] Force-killed process PID=${processId} (partition: ${partitionId || 'unknown'})`);
    // Log the forced kill for diagnostics
    if (typeof window.logForceKill === 'function') {
      window.logForceKill(processId, partitionId);
    }
  } catch (err) {
...
window.__waitForWebviewExit = function(webview, timeoutMs = 15000) {
  return new Promise((resolve) => {
    if (!webview) return resolve('no-webview');
    let settled = false;
    const done = (reason) => {
      if (settled) return;
      settled = true;
      try {
        webview.removeEventListener('exit', onExit);
        webview.removeEventListener('destroyed', onDestroyed);
        webview.removeEventListener('crashed', onCrashed);
        webview.removeEventListener('close', onClose);
      } catch (e) {}
      resolve(reason);
    };
    const onExit = () => done('exit');
    const onDestroyed = () => done('destroyed');
    const onCrashed = () => done('crashed');
    const onClose = () => done('close');

    try {
      webview.addEventListener('exit', onExit);
      webview.addEventListener('destroyed', onDestroyed);
      webview.addEventListener('crashed', onCrashed);
      webview.addEventListener('close', onClose);
    } catch (e) {}
...
/**
 * Đợi webview terminate và VERIFY process thực sự đã chết
 * QUAN TRỌNG: Chỉ resolve khi process VERIFIED đã chết
 */
window.waitForWebviewCompleteTermination = async function(webview, processId, timeoutMs = 20000) {
  if (!webview) return { success: true, reason: 'no-webview' };
  
  const startTime = Date.now();
  console.log(`[WebviewTermination] ⏳ Đợi webview terminate (PID: ${processId}, timeout: ${timeoutMs}ms)...`);
  
  // Step 1: Đợi webview event (exit/destroyed/crashed)
  const eventReason = await window.__waitForWebviewExit(webview, timeoutMs);
  const eventTime = Date.now() - startTime;
  console.log(`[WebviewTermination] 📡 Event '${eventReason}' fired sau ${eventTime}ms`);
  
  // Step 2: VERIFY process thực sự đã chết (quan trọng!)
  if (processId && window.hasOwnProperty('process')) {
    const remainingTime = Math.max(1000, timeoutMs - eventTime);
    console.log(`[WebviewTermination] 🔍 Verify process ${processId} đã chết (timeout: ${remainingTime}ms)...`);
    
    const processDead = await window.waitForProcessDeath(processId, remainingTime, 200);
    
    if (processDead) {
      const totalTime = Date.now() - startTime;
      console.log(`✅ [WebviewTermination] Process ${processId} VERIFIED chết sau ${totalTime}ms`);
      return { success: true, reason: 'verified-dead', time: totalTime };
    } else {
      console.warn(`⚠️ [WebviewTermination] Process ${processId} VẪN CHẠY sau ${timeoutMs}ms!`);
      
      // Step 3: Force kill nếu process vẫn chạy
      if (window.WEBVIEW_FORCE_KILL_ON_CLOSE) {
        console.warn(`🔪 [WebviewTermination] Force killing process ${processId}...`);
        window.__forceKillWebviewProcess(processId);
        
        // Step 4: Verify lại sau khi force kill
        const verifyDead = await window.waitForProcessDeath(processId, 5000, 200);
        if (verifyDead) {
          const totalTime = Date.now() - startTime;
          console.log(`✅ [WebviewTermination] Process ${processId} chết sau force kill (${totalTime}ms)`);
          return { success: true, reason: 'force-killed', time: totalTime };
        } else {
          const totalTime = Date.now() - startTime;
          console.error(`❌ [WebviewTermination] Process ${processId} KHÔNG THỂ KILL sau ${totalTime}ms!`);
          return { success: false, reason: 'unkillable', time: totalTime };
        }
      } else {
        return { success: false, reason: 'still-running', time: Date.now() - startTime };
      }
    }
  } else {
    // Không có process ID để verify -> tin vào event
    return { success: true, reason: eventReason, time: eventTime };
  }
};

/**
 * SIMPLE WEBVIEW CLEANUP - Single pass when tab lifetime expires
 * Skips locked files, no retries, no blocking
 * Used when webview auto_close triggers
 */
window.simpleWebviewCleanup = async function(id_tab, partitionId) {
  if (!partitionId || !window.hasOwnProperty('process')) {
  ...
  try {
    ...
    const foldersToClean = [
      Path.join(userDataRoot, 'Partitions', partitionId),
      Path.join(userDataRoot, 'Local Storage', partitionId),
      Path.join(userDataRoot, 'Cache', partitionId),
      Path.join(userDataRoot, 'IndexedDB', partitionId),
      Path.join(userDataRoot, 'Session Storage', partitionId),
    ];
    
    let totalSkipped = 0;
    console.log(`[simpleWebviewCleanup] 🗑️ Cleanup partition ${partitionId}`);
    
    foldersToClean.forEach((folderPath) => {
      const result = deleteFolderSimple(folderPath);
      if (result.deleted) {
        console.log(`   ✅ Deleted: ${Path.basename(folderPath)}`);
      } else if (result.skipped > 0) {
        console.log(`   ⚠️ Skipped ${result.skipped} locked files`);
        totalSkipped += result.skipped;
      }
    });
    
    return { success: true, skipped: totalSkipped };
  } catch (err) {
    console.error(`[simpleWebviewCleanup] Error:`, err.message);
    return { success: false, error: err.message };
  }
};

/**
 * MAIN CLEANUP - Xóa TOÀN BỘ dữ liệu của webview
 */
window.fnCompleteWebviewCleanup = function(id_tab, partitionId, webviewId) {
  return new Promise(async (resolve) => {
    console.log(`\n🧹 [COMPLETE CLEANUP START] ${id_tab} (partition: ${partitionId})`);
    const cleanupStart = Date.now();
    let totalItemsRemoved = 0;
    let totalSizeFreed = 0;
    
    try {
      // Bước 1: Xóa từ webview API
      console.log(`  [1] Webview API cleanup...`);
      const webview = document.querySelector('#' + webviewId);
      if (webview && typeof webview.clearData === 'function') {
        try {
          webview.clearData({ since: 0 }, {
            appcache: true, cache: true, cookies: true,
            fileSystems: true, indexedDB: true,
            localStorage: true, webSQL: true
          });
          console.log(`    ✅ Cleared (cache, cookies, localStorage, indexedDB, etc)`);
          totalItemsRemoved += 7;
        } catch (e) {
      ...
      // Bước 2: Clear service workers
      console.log(`  [2] Service workers cleanup...`);
      ...
      // Bước 3: Clear cache storage
      console.log(`  [3] Cache storage cleanup...`);
      ...
      // Bước 4: OS-level cleanup
      if (partitionId && window.hasOwnProperty('process')) {
        // macOS cần thời gian dài hơn để release file handles
        // Increased from 10s to 15s for macOS to ensure file locks are released
        const cleanupDelay = typeof process !== 'undefined' && process.platform === 'darwin' ? 15000 : 8000;
        console.log(`  [4] OS-level partition cleanup (waiting ${cleanupDelay/1000}s for API cleanup to complete)...`);
        console.log(`  [4] ⏰ Cleanup scheduled for: ${new Date(Date.now() + cleanupDelay).toLocaleTimeString()}`);
        setTimeout(() => {
          console.log(`  [4] ⏰ Executing scheduled cleanup NOW (${cleanupDelay/1000}s elapsed)`);
          if (typeof window.deletePartitionFilesCompletely === 'function') {
            // Lấy tabLifetimeMinutes từ tracker (nếu có)
            const webviewInfo = window.__webviewStorageTracker.activeWebviews.get(id_tab);
            const tabLifetimeMinutes = webviewInfo ? webviewInfo.tabLifetimeMinutes : null;
            window.deletePartitionFilesCompletely(partitionId, tabLifetimeMinutes);
            console.log(`    ✅ OS cleanup for partition: ${partitionId} (tab lifetime: ${tabLifetimeMinutes || 'default'} phút)`);
          } else {
            console.error(`    ❌ deletePartitionFilesCompletely function not found!`);
          }
        }, cleanupDelay);
      }
      
      const cleanupDuration = Date.now() - cleanupStart;
      console.log(`✅ Cleanup done in ${cleanupDuration}ms\n`);
      
      window.__webviewStorageTracker.logCleanup(id_tab, partitionId, totalItemsRemoved, totalSizeFreed);
      resolve({ success: true, itemsRemoved: totalItemsRemoved, sizeFreed: totalSizeFreed, duration: cleanupDuration });
...
// Cleanup queue to serialize heavy OS deletes and avoid overload
if (!window.__cleanupQueue) {
  window.__cleanupQueue = {
    pending: [],
    running: 0,
    maxConcurrent: 1,
    enqueue: function(task, label) {
      return new Promise((resolve) => {
        this.pending.push({ task, resolve, label });
        this.pump();
      });
    },
    pump: function() {
      while (this.running < this.maxConcurrent && this.pending.length > 0) {
        ...
        Promise.resolve()
          ...
          .catch((err) => {
            this.running--;
            item.resolve({ success: false, error: err?.message || String(err) });
            this.pump();
          });
      }
    },
...
window.enqueueWebviewCleanup = function(id_tab, partitionId, webviewId) {
  if (!window.__cleanupQueue || typeof window.fnCompleteWebviewCleanup !== 'function') {
    return Promise.resolve({ success: false, error: 'cleanup_queue_not_ready' });
  }
  return window.__cleanupQueue.enqueue(
    () => window.fnCompleteWebviewCleanup(id_tab, partitionId, webviewId),
    `tab:${id_tab}`
  );
};

/**
 * Hàm kiểm tra webviews hiện tại
 */
window.fnDiagnoseWebviews = function() {
  console.log(`\n╔════════════════════════════════════════════════════════════════╗`);
  console.log(`║  WEBVIEW DIAGNOSTIC REPORT`);
  console.log(`╚════════════════════════════════════════════════════════════════╝\n`);
  
  const webviews = document.querySelectorAll('webview');
  console.log(`📊 Active DOM Webviews: ${webviews.length}`);
  
  webviews.forEach((wv, idx) => {
    const id = wv.getAttribute('id');
    const partition = wv.getAttribute('partition');
    const src = wv.getAttribute('src');
    console.log(`  ${idx + 1}. [${id}] Partition: ${partition} | URL: ${src}`);
  });
  
  console.log(`\n📋 Tracker Statistics:`);
  console.log(`  Active Webviews: ${window.__webviewStorageTracker.activeWebviews.size}`);
  console.log(`  Total Cleaned: ${(window.__webviewStorageTracker.totalCleaned / 1024 / 1024).toFixed(2)} MB`);
  console.log(`  Operations: ${window.__webviewStorageTracker.cleanupLog.length}`);
  
  window.__webviewStorageTracker.printReport();
  console.log(`\n`);
};

/**
 * COMPREHENSIVE STORAGE DIAGNOSTIC
 * Runs all diagnostic checks and provides actionable insights
 */
window.diagnoseStorageIssue = function() {
  console.log(`\n`);
  console.log(`╔═══════════════════════════════════════════════════════════════════════╗`);
  console.log(`║                                                                       ║`);
  console.log(`║            🔍 COMPREHENSIVE WEBVIEW STORAGE DIAGNOSTIC 🔍            ║`);
  console.log(`║                                                                       ║`);
  console.log(`╚═══════════════════════════════════════════════════════════════════════╝`);
  console.log(`\n`);
  
  // 1. Active webviews in DOM
  const activeWebviews = document.querySelectorAll('webview[id^="U_"]');
  console.log(`📊 SECTION 1: ACTIVE WEBVIEWS`);
  console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  console.log(`   Count: ${activeWebviews.length}`);
  if (activeWebviews.length > 0) {
    activeWebviews.forEach((wv, idx) => {
      const id = wv.id;
      const partition = wv.getAttribute('partition') || wv.dataset.partitionId;
      console.log(`   ${idx + 1}. ${id} → ${partition}`);
    });
  } else {
    console.log(`   ✅ No active webviews (good for testing cleanup)`);
  }
  console.log(`\n`);
  
  // 2. Cleanup call log
   console.log(`📋 SECTION 2: CLEANUP INVOCATION LOG`);
  console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  if (window.__deletePartitionCallLog && window.__deletePartitionCallLog.length > 0) {
    console.log(`   Total cleanup calls: ${window.__deletePartitionCallLog.length}`);
    console.log(`   Last 5 calls:`);
    const recent = window.__deletePartitionCallLog.slice(-5);
    recent.forEach((call, idx) => {
      console.log(`   ${idx + 1}. ${call.timestamp} → ${call.partitionId}`);
    });
  } else {
    console.log(`   ⚠️  NO CLEANUP CALLS LOGGED!`);
    console.log(`   This means deletePartitionFilesCompletely() has NEVER been called.`);
    console.log(`   Possible reasons:`);
    console.log(`     - AUTO_CLEANUP_ON_CLOSE is disabled`);
    console.log(`     - fnRemoveTab() not working properly`);
    console.log(`     - Cleanup scheduled but not executed yet (timeout delay)`);
  }
  console.log(`\n`);
  
  // 3. Orphaned partitions
  console.log(`💾 SECTION 3: ORPHANED PARTITIONS`);
  console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  const orphanedInfo = window.listOrphanedPartitions();
  console.log(`\n`);
  
  // 4. Cleanup config
  console.log(`⚙️  SECTION 4: CLEANUP CONFIGURATION`);
  console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  console.log(`   AUTO_CLEANUP_ON_STARTUP: ${window.AUTO_CLEANUP_ON_STARTUP !== false ? '✅ Enabled' : '❌ Disabled'}`);
  console.log(`   AUTO_CLEANUP_ON_CLOSE: ${window.AUTO_CLEANUP_ON_CLOSE !== false ? '✅ Enabled' : '❌ Disabled'}`);
  console.log(`   Cache cleanup interval: ${window.__cacheCleanupInterval ? '✅ Running (every 30s)' : '❌ Not running'}`);
  console.log(`\n`);
  ...
  const hasOrphaned = orphanedInfo.orphanedPartitions && orphanedInfo.orphanedPartitions.length > 0;
  const hasNoCleanupCalls = !window.__deletePartitionCallLog || window.__deletePartitionCallLog.length === 0;
  const cleanupDisabled = window.AUTO_CLEANUP_ON_CLOSE === false;
  
  if (cleanupDisabled) {
    console.log(`   ⚠️  AUTO_CLEANUP_ON_CLOSE is DISABLED!`);
    console.log(`   → Enable it: window.AUTO_CLEANUP_ON_CLOSE = true;`);
  } else if (hasNoCleanupCalls) {
    console.log(`   ⚠️  Cleanup is enabled but NEVER called!`);
    console.log(`   → Try closing a tab and check if cleanup runs`);
    console.log(`   → Check console for: [fnRemoveTab], [deletePartitionFilesCompletely]`);
    console.log(`   → If no logs appear, fnRemoveTab() may have errors`);
  } else if (hasOrphaned) {
    console.log(`   ⚠️  Cleanup IS running but partitions still orphaned!`);
    console.log(`   → Cleanup calls: ${window.__deletePartitionCallLog.length}`);
    console.log(`   → Orphaned folders: ${orphanedInfo.orphanedPartitions.length}`);
    console.log(`   → Possible causes:`);
    console.log(`       - File locks not released (macOS delay too short)`);
    console.log(`       - Process not terminating properly`);
    console.log(`       - Rapid tab open/close causing timing issues`);
    console.log(`   → Try: Close all tabs, wait 30s, then run window.cleanupAllPartitions()`);
  } else {
    console.log(`   ✅ Everything looks good!`);
    console.log(`   → Cleanup is enabled and running`);
    console.log(`   → No orphaned partitions found`);
    console.log(`   → ${window.__deletePartitionCallLog.length} cleanup calls logged`);
  }
  
  console.log(`\n`);
  console.log(`📝 QUICK ACTIONS:`);
  console.log(`   - List orphaned: window.listOrphanedPartitions()`);
  console.log(`   - View cleanup log: window.getCleanupCallLog()`);
  console.log(`   - Force cleanup all: window.cleanupAllPartitions()`);
  console.log(`   - Check webviews: window.fnDiagnoseWebviews()`);
  console.log(`\n`);
  
  return {
    activeWebviews: activeWebviews.length,
    cleanupCalls: window.__deletePartitionCallLog ? window.__deletePartitionCallLog.length : 0,
    orphanedPartitions: orphanedInfo.orphanedPartitions ? orphanedInfo.orphanedPartitions.length : 0,
    cleanupEnabled: window.AUTO_CLEANUP_ON_CLOSE !== false,
    recommendations: {
      cleanupDisabled,
      hasNoCleanupCalls,
      hasOrphaned
  ...
  window.fnRemoveTab = function (id_tab) {
    try {
        ...
        if (webview) {
            // Lấy partition ID từ webview attribute - GIỮ 'persist:' prefix
            const partition = webview.getAttribute('partition');
            if (partition) {
                partitionId = partition; // Keep full 'persist:xxxxx' format
            ...
            try {
                ...
                // Clear all data before setting to about:blank
                try {
                    webview.clearData(
                        { since: 0 },
                        {
                    ...
                    console.log(`[fnRemoveTab] ✅ Cleared webview data`);
                } catch (clearDataErr) {
                    console.warn(`[fnRemoveTab] ⚠️ clearData failed:`, clearDataErr.message);
                }
                
                // Set blank để giải phóng tài nguyên
                webview.src = 'about:blank';
                
                // Wait a moment for about:blank to load
                setTimeout(() => {
                    console.log(`[fnRemoveTab] ℹ️ Webview redirected to about:blank`);
                }, 100);
            } catch (stopErr) {
                console.warn(`[fnRemoveTab] ⚠️ Lỗi khi stop webview:`, stopErr.message);
            }
            
            // BƯỚC 2: Remove tất cả event listeners
            ...
            // BƯỚC 3: Terminate webview process (quan trọng!)
            console.log(`[fnRemoveTab] 💀 Terminate webview process (PID: ${processId})...`);
            try {
                if (typeof webview.terminate === 'function') {
                    webview.terminate();
            ...
            // BƯỚC 4: ⚠️ QUAN TRỌNG - ĐỢI VÀ VERIFY PROCESS ĐÃ CHẾT HOÀN TOÀN
            // Chỉ cleanup SAU KHI verify process thực sự đã terminate
            console.log(`[fnRemoveTab] ⏳ Waiting for VERIFIED process termination...`);
            
            window.waitForWebviewCompleteTermination(webview, processId, 25000).then(async (result) => {
              console.log(`[fnRemoveTab] 🏁 Termination result:`, result);
              
              if (!result.success) {
                console.error(`[fnRemoveTab] ❌ CẢNH BÁO: Process ${processId} KHÔNG TERMINATE được!`);
                console.error(`[fnRemoveTab] ❌ Lý do: ${result.reason}`);
                console.error(`[fnRemoveTab] ❌ Files có thể KHÔNG XÓA ĐƯỢC do process vẫn lock!`);
                
                // Vẫn thử cleanup nhưng có thể thất bại
                if (typeof window.markForDeferredCleanup === 'function' && partitionId) {
                  // Mark partition folders để cleanup khi restart
                  const Path = require('path');
                  const gui = require('nw.gui');
                  const userDataPath = gui.App.dataPath;
                  const userDataRoot = (typeof window.getWebviewUserDataRoot === 'function'
                  ...
                  foldersToMark.forEach(path => window.markForDeferredCleanup(path));
                }
              } else {
                console.log(`✅ [fnRemoveTab] Process ${processId} VERIFIED chết - An toàn để cleanup!`);
              }

              // Remove DOM element sau khi process đã chết (hoặc force killed)
              try {
                console.log(`[fnRemoveTab] 🗑️ Remove DOM element...`);
                webview.remove();
              } catch (e) {}

              // 🧹 QUEUED FULL CLEANUP: Serialize OS deletes to avoid overload
              console.log(`[fnRemoveTab] 🧹 Queue full cleanup for partition: ${partitionId || 'unknown'}`);
              try {
                if (typeof window.enqueueWebviewCleanup === 'function') {
                  window.enqueueWebviewCleanup(id_tab, partitionId, webviewId).then((cleanupResult) => {
                    if (cleanupResult?.success) {
                      console.log(`[fnRemoveTab] ✅ Cleanup done for ${id_tab}`);
                    } else if (cleanupResult?.error) {
                      console.warn(`[fnRemoveTab] ⚠️ Cleanup error for ${id_tab}: ${cleanupResult.error}`);
                    }
                  });
                } else if (typeof window.fnCompleteWebviewCleanup === 'function') {
                  window.fnCompleteWebviewCleanup(id_tab, partitionId, webviewId).then((cleanupResult) => {
                    if (cleanupResult?.success) {
                      console.log(`[fnRemoveTab] ✅ Cleanup done for ${id_tab}`);
                    }
                  });
                } else {
                  const cleanupResult = await window.simpleWebviewCleanup(id_tab, partitionId);
                  if (cleanupResult.success) {
                    console.log(`[fnRemoveTab] ✅ Cleanup done - skipped ${cleanupResult.skipped} locked files`);
                  } else {
                    console.warn(`[fnRemoveTab] ⚠️ Cleanup error: ${cleanupResult.error}`);
                  }
                }
              } catch (cleanupErr) {
                console.warn(`[fnRemoveTab] ⚠️ Cleanup exception: ${cleanupErr.message}`);
              }
            });
        }
        
        // 6. Xóa panelItem (container chứa header, content, webview)
        ...
        // 9. UNREGISTER từ tracker
        console.log(`[fnRemoveTab] 📊 Unregistering from tracker...`);
        window.__webviewStorageTracker.unregisterWebview(id_tab);
  ...
  window.stopApp = function (cleanupTabs = true) {
    ...
    // Đánh dấu app đã dừng NGAY LẬP TỨC để các interval tự dừng
    isRunning = false;
    window.isRunning = false;
    ...
    if (window.tmRun) {
      clearInterval(window.tmRun);
      window.tmRun = null;
      console.log('[stopApp] ✅ Stopped tmRun interval');
    }
    
    // 2. Dừng health monitor
    window.stopWebviewHealthMonitor();
    console.log('[stopApp] 🏥 Đã dừng webview health monitor');
    
    // 3. Lưu lại trạng thái UnifiedLinkManager và TabManager
    const stats = window.UnifiedLinkManager.getStats();
    ...
    // 4. TẮT PROXY NGAY LẬP TỨC (trước khi đóng tabs)
  ...
  window.runApp = function () {
    ...
    // Clear existing interval nếu có để tránh chồng chéo
    if (window.tmRun) {
      console.log('[runApp] ⚠️ Clearing existing tmRun interval');
      clearInterval(window.tmRun);
      window.tmRun = null;
    }
    ...
    window.tmRun = setInterval(function () {
      ...
      // Kiểm tra nếu app đã dừng, clear interval
      if (!isRunning) {
        console.log('[tmRun] ⚠️ App đã dừng, clearing interval');
        clearInterval(window.tmRun);
        window.tmRun = null;
        return;
      }
      
      // 🏥 KIỂM TRA PROXY HEALTH - Kiểm tra thực tế proxy còn hoạt động không
      // Chạy async không block interval
      if (typeof window.checkProxyHealth === 'function') {
        window.checkProxyHealth().then(result => {
          if (!result.healthy && result.reason !== 'skip' && result.reason !== 'throttled') {
            console.warn(`⚠️ [tmRun] Proxy health check failed: ${result.reason}`);
          }
        }).catch(err => {
          console.error('[tmRun] Proxy health check error:', err);
        });
      }
      
      // � KIỂM TRA PROXY CẦN RESET (do health check hoặc auth error)
      if (window.__proxyNeedsReset) {
        console.error('🚨 [tmRun] Proxy CẦN RESET (health check failed hoặc auth error)! Bắt đầu force reset...');
        
        const activeTabs = document.querySelectorAll('[id^="U_"]:not([id*="tabSetting"])').length;
        if (activeTabs > 0) {
          console.log(`🗑️ [tmRun] Đóng ${activeTabs} tabs do proxy cần reset...`);
          closeAllTabsAndCleanup('Proxy failed health check');
        }
        
        // Reset proxy ngay lập tức
        window.__isProxyActive = false;
        window.__proxyActivatedTime = 0;
        setTimeout(() => {
          window.__proxyNeedsReset = false;
      ...
      // �🔴 KIỂM TRA PROXY HẾT HẠN (60 phút) - QUAN TRỌNG!
      if (typeof window.isProxyExpired === 'function' && window.isProxyExpired()) {
        console.error('🚨 [tmRun] Proxy đã HẾT HẠN (60 phút)! Bắt đầu force reset...');
        
        const activeTabs = document.querySelectorAll('[id^="U_"]:not([id*="tabSetting"])').length;
        if (activeTabs > 0) {
          console.log(`🗑️ [tmRun] Đóng ${activeTabs} tabs do proxy hết hạn...`);
          closeAllTabsAndCleanup('Proxy hết hạn sau 60 phút');
        }
    ...
    }, 2000); // Check mỗi 2 giây thay vì getStayMinutes() * 1000
    return false;
  }
