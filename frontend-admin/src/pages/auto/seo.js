...
console.log('[afterDelete] ✅ Deleted row, syncing', Array.isArray(allData) ? allData.length : 0, 'total rows');
if (!Array.isArray(allData)) {
  console.error('[afterDelete] ERROR: allData is not an array!', allData);
  return;
}
window.dataUserOption = allData;
if (window.csmUserData && typeof window.csmUserData.set === 'function') {
  console.log('[afterDelete] Calling csmUserData.set()...');
  window.csmUserData.set(allData, function(success, error) {
    if (success) {
      console.log('[afterDelete] ✅ Saved to csmUserData, count:', allData.length);
    } else {
      console.error('[afterDelete] ❌ csmUserData.set failed:', error);
      if (window.canhbao) {
        window.canhbao('Lưu user_address sau khi xóa thất bại: ' + (error || 'Unknown error'));
      }
    }
  });
} else {
