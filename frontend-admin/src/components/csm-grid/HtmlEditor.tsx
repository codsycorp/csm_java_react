      ...
      try {
        ...
        reader.onload = async () => {
          try {
            ...
            // Upload to backend with JSON body
            const response = await fetch(UPLOAD_ENDPOINT, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ app_id: currentAppId, name: fileName, src: dataUrl }),
            });
            ...
            const resolveImagePath = (pathStr: string): string => {
              if (!pathStr) return `/images.shtml?app_id=${currentAppId}&name=${fileName}`;
              ...
              if (match) {
                const [, aid, name] = match;
                return `/images.shtml?app_id=${aid}&name=${name}`;
              }
              return `/images.shtml?app_id=${currentAppId}&name=${pathStr}`;
            };
            ...
            if (responseJson) {
              ...
              viewUrl = responseText.includes('app_images/') 
                ...
                : `/images.shtml?app_id=${currentAppId}&name=${fileName}`;
            }
