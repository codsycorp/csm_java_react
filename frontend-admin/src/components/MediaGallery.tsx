...
const resolveMediaUrl = (pathValue: string): string => {
  if (!pathValue) return "";
...
const MediaGallery: React.FC<MediaGalleryProps> = ({ images = [], videos = [], alt, showCount = true }) => {
  ...
  const mediaItems: MediaItem[] = [
    ...images.filter(Boolean).map(url => ({ url: resolveMediaUrl(url), type: 'image' as const })),
    ...videos.filter(Boolean).map(url => ({ url: resolveMediaUrl(url), type: 'video' as const })),
  ];
