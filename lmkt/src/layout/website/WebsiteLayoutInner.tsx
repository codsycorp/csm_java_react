...
export default function WebsiteLayoutInner({ children, selectedKey, menuItems, title, breadcrumb }: WebsiteLayoutProps) {
  ...
  // Listen for phone changes from other components (e.g., contact modal in wu_service_detail)
  useEffect(() => {
    const handlePhoneChange = (event: Event) => {
      const detail = (event as CustomEvent).detail;
      if (detail?.appId === appId && typeof detail.phone === 'string') {
        // If modal is open and we receive a phone, close it
        if (showNameInput && detail.phone.trim()) {
          setShowNameInput(false);
        }
      }
    };

    window.addEventListener('csm-guest-phone-changed', handlePhoneChange as EventListener);
    return () => window.removeEventListener('csm-guest-phone-changed', handlePhoneChange as EventListener);
  }, [appId, showNameInput]);
  ...
  return (
    <ConfigProvider theme={themeConfig}>
      ...
      <Modal
        ...
        onCancel={() => {
          setShowNameInput(false);
          setInputPhone(""); // Clear local input
          pendingMessageRef.current = null; // Clear pending message khi cancel
        }}
        okText={t('common.chat.startChat')}
        cancelText={t('common.cancel')}
        afterOpenChange={(open) => {
          if (open) {
            // When modal opens, pre-fill inputPhone from guestPhone if available
