...
// import { Iframe } from "#src/components/iframe"; // Not available in lmkt
...
export function getMenuItems(routeList: AppRouteRecordRaw[]) {
		// Tạo map để loại bỏ trùng key
		const keyMap = new Map<string, MenuItemType>();
		routeList.forEach(item => {
			...
			if (!item?.handle?.hideInMenu && menuItem.key && !keyMap.has(menuItem.key)) {
				keyMap.set(menuItem.key, menuItem);
			}
		});
...
export function addAsyncRoutes(arrRoutes: Array<AppRouteRecordRaw>) {
	...
	arrRoutes.forEach((v: AppRouteRecordRaw) => {
		...
		// 	v.Component = Iframe; // Iframe not available in lmkt
		// }
