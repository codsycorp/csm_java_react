...
const BLOCKED_WEBSITE_ROUTES = new Set(["/cong-cu", "/xem-ngay", "/kqxs"]);
...
export function isWebsiteMode() {
	// LMKT is website-only, no admin mode
	return true;
}
...
function isWebsiteRoute(pathname: string) {
	...
	// Các URL khác sẽ được xử lý bởi backend (redirect về trang chủ)
	return false;
}
...
export async function routerInitReady(reactRouter: ReactRouterType) {
	...
	// Kiểm tra hợp lệ cho route /:category/:slug khi reload hoặc nhập URL trực tiếp (clean URLs, no .shtml)
	// CHỈ validate category exists trong SSR, KHÔNG validate slug vì slug là dynamic từ database
	...
	if (pathnameWithoutBase === "/login") {
		...
		if (redirectParam === "admin") {
			// LMKT: No admin, redirect to website home
			reactRouter.navigate("/", { replace: true });
		}
