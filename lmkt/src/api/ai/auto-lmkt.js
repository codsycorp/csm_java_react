...
async function fetchAllTableRowsPaginated({ appId, objName, where, pageSize = 200, onBatch }) {
  if (!window.csmApi?.getTableData) {
    throw new Error("window.csmApi.getTableData không khả dụng");
  }
  let lastkey;
  const all = [];
  let page = 0;
  while (true) {
    const res = await window.csmApi.getTableData({
      app_id: appId,
      obj_name: objName,
...
function ensureGlobalSettingsPanel() {
  ...
  wrapper.append(title, settingsContainer, domainMigrationSection, facebookCommonSection);
