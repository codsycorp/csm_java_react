...
function shouldMigrateRowDomain(domainValue, targetDomainKey) {
  ...
  const current = String(domainValue || "").trim();
...
function summarizeDomainMigrationRows(rows, targetDomainKey) {
  const canonical = canonicalDomainForKey(targetDomainKey);
  const byOld = {};
  const pending = [];
  (rows || []).forEach((row) => {
    const from = String(row.domain || "").trim() || "(trống)";
    if (!shouldMigrateRowDomain(row.domain, targetDomainKey)) return;
    byOld[from] = (byOld[from] || 0) + 1;
    pending.push(row);
  });
  return { canonical, byOld, pending, totalScanned: (rows || []).length };
}
...
async function scanDomainMigrationInventory(domainKey, options = {}) {
  ...
  const report = {
    ...
    scopeServiceType: scopeServiceType || null,
    web_service_detail: { totalScanned: 0, pending: 0, byOld: {} },
    web_services: { totalScanned: 0, pending: 0, byOld: {} },
    zalo_configs: { totalScanned: 0, pending: 0, byOld: {} },
    totalPending: 0
  };
  ...
  if (options.includeDetail !== false) {
    ...
    report.web_service_detail = {
      totalScanned: s.totalScanned,
      pending: s.pending.length,
      byOld: s.byOld,
      _pendingRows: s.pending
    };
  }
  ...
  if (options.includeZaloConfigs !== false && typeof loadDataOptionUser === "function") {
    const configs = loadDataOptionUser().filter((x) => x && x.config_for_zalo);
    const byOld = {};
    const pending = [];
    configs.forEach((cfg) => {
      const from = String(cfg.domain || "").trim() || "(trống)";
      if (!shouldMigrateRowDomain(cfg.domain, domainKey)) return;
      if (scopeServiceType && String(cfg.service_type || "") !== scopeServiceType) return;
      byOld[from] = (byOld[from] || 0) + 1;
      pending.push(cfg);
    });
    report.zalo_configs = {
      totalScanned: configs.length,
  ...
  report.totalPending =
    report.web_service_detail.pending
    + report.web_services.pending
    + report.zalo_configs.pending;

  return report;
}

function formatDomainMigrationReportText(report) {
  const lines = [
    `🎯 Domain đích (${report.domainKey}): ${report.targetDomain}`,
    report.scopeServiceType ? `🔎 Lọc service_type/project: ${report.scopeServiceType}` : "🔎 Phạm vi: toàn bộ app",
    "",
    `📰 web_service_detail: ${report.web_service_detail.pending}/${report.web_service_detail.totalScanned} cần đổi`,
    ...Object.entries(report.web_service_detail.byOld || {}).map(([d, n]) => `   • ${d} → ${n} bản ghi`),
    "",
    `📁 web_services: ${report.web_services.pending}/${report.web_services.totalScanned} cần đổi`,
    ...Object.entries(report.web_services.byOld || {}).map(([d, n]) => `   • ${d} → ${n} bản ghi`),
    "",
    `⚙️ Cấu hình Zalo: ${report.zalo_configs.pending}/${report.zalo_configs.totalScanned} cần đổi`,
    ...Object.entries(report.zalo_configs.byOld || {}).map(([d, n]) => `   • ${d} → ${n} config`),
    "",
    `📊 Tổng cộng: ${report.totalPending} mục cần migrate`
  ];
  return lines.join("\n");
}
...
async function executeDomainMigration(report, options = {}) {
  ...
  const domainKey = report.domainKey;
  const targetDomain = report.targetDomain;
  ...
  if (report.web_service_detail._pendingRows?.length) {
    await migrateRows(
      report.web_service_detail._pendingRows,
      "web_service_detail",
      ["slug", "domain", "status"],
      "detail"
    );
  }
  if (report.web_services._pendingRows?.length) {
    await migrateRows(
      report.web_services._pendingRows,
      "web_services",
      ["slug", "domain", "status"],
      "services"
    );
  ...
  if (report.zalo_configs._pendingRows?.length && typeof loadDataOptionUser === "function") {
    const pendingIds = new Set(report.zalo_configs._pendingRows.map((c) => c.id));
    const all = loadDataOptionUser();
    let changed = 0;
    ...
    if (!dryRun && changed > 0) {
      await new Promise((resolve, reject) => {
        saveDataOptionUser(next, (ok, err) => (ok ? resolve() : reject(new Error(err || "Lưu Zalo config thất bại"))));
      });
    }
    stats.zalo = changed;
  }
...
function ensureDomainMigrationPanel(theme) {
  if (!theme) theme = getThemeTokens();
  let panel = document.getElementById("domain-migration-panel");
  if (panel) return panel;

  panel = document.createElement("div");
  panel.id = "domain-migration-panel";
  panel.style.cssText = `margin-top:14px;padding:12px;border:1px solid ${theme.border};border-radius:8px;background:${theme.surface || theme.bg};`;

  const title = document.createElement("div");
  title.style.cssText = `font-weight:700;margin-bottom:6px;color:${theme.text};font-size:13px;`;
  title.textContent = ti(
    "🔁 Chuẩn hóa domain (dữ liệu cũ → mới)",
    "🔁 Domain normalization (legacy → canonical)",
    "🔁 域名规范化（旧数据 → 新标准）"
  );
  ...
  targetPreview.id = "domain-migration-target";
  targetPreview.style.cssText = `font-size:12px;margin-bottom:8px;color:${theme.text};`;

  const scopeRow = document.createElement("label");
  scopeRow.style.cssText = `display:flex;align-items:center;gap:6px;font-size:12px;color:${theme.text};margin-bottom:8px;cursor:pointer;`;
  const scopeOnlyCurrent = document.createElement("input");
  scopeOnlyCurrent.type = "checkbox";
  scopeOnlyCurrent.id = "domain-migration-scope-current";
  scopeOnlyCurrent.checked = false;
  scopeRow.appendChild(scopeOnlyCurrent);
  scopeRow.appendChild(document.createTextNode(ti(
    "Chỉ lĩnh vực/dự án đang chọn (service_type)",
    "Only current industry/project (service_type)",
    "仅当前行业/项目 (service_type)"
  )));

  const checkWrap = document.createElement("div");
  checkWrap.style.cssText = "display:flex;flex-wrap:wrap;gap:12px;margin-bottom:10px;font-size:12px;color:" + theme.text;
  ["detail", "services", "zalo"].forEach((key) => {
    const lbl = document.createElement("label");
    lbl.style.cssText = "display:flex;align-items:center;gap:4px;cursor:pointer;";
    const cb = document.createElement("input");
    cb.type = "checkbox";
    cb.id = `domain-migration-include-${key}`;
    cb.checked = true;
    lbl.appendChild(cb);
    const labels = {
      detail: ti("Bài chi tiết", "Detail posts", "详情文章"),
      services: ti("Danh mục web_services", "web_services categories", "web_services 分类"),
      zalo: ti("Config Zalo", "Zalo configs", "Zalo 配置")
    };
    lbl.appendChild(document.createTextNode(labels[key]));
  ...
  const scanBtn = createButton(ti("🔍 Quét dữ liệu cũ", "🔍 Scan legacy data", "🔍 扫描旧数据"), "#595959");
  const dryRunBtn = createButton(ti("🧪 Chạy thử (không lưu)", "🧪 Dry run", "🧪 试运行"), "#fa8c16");
  const migrateBtn = createButton(ti("✅ Migrate & lưu DB", "✅ Migrate & save", "✅ 迁移并保存"), "#1677ff");

  const logPre = document.createElement("pre");
  logPre.id = "domain-migration-log";
  logPre.style.cssText = `margin:0;padding:10px;max-height:220px;overflow:auto;font-size:11px;line-height:1.45;white-space:pre-wrap;border:1px solid ${theme.border};border-radius:6px;background:${theme.inputBg};color:${theme.text};`;

  const refreshTargetLabel = () => {
    const gs = getGlobalSettings();
    const canonical = canonicalDomainForKey(gs.domainKey);
    targetPreview.textContent = ti(
      `Domain đích: ${canonical} (key: ${gs.domainKey})`,
      `Target domain: ${canonical} (key: ${gs.domainKey})`,
      `目标域名：${canonical}（key：${gs.domainKey}）`
    );
  };

  const appendLog = (text) => {
    const ts = new Date().toLocaleTimeString();
    logPre.textContent = `[${ts}] ${text}\n\n${logPre.textContent || ""}`.trim();
  };
  ...
  scanBtn.onclick = async () => {
    refreshTargetLabel();
    const gs = getGlobalSettings();
    scanBtn.disabled = dryRunBtn.disabled = migrateBtn.disabled = true;
    logPre.textContent = "";
    appendLog(ti("Đang quét...", "Scanning...", "扫描中..."));
    try {
      const scopeServiceType = scopeOnlyCurrent.checked
        ? (gs.isLmkt ? gs.project : gs.industry)
        : "";
      lastReport = await scanDomainMigrationInventory(gs.domainKey, {
        scopeServiceType,
        includeDetail: document.getElementById("domain-migration-include-detail")?.checked !== false,
        includeServices: document.getElementById("domain-migration-include-services")?.checked !== false,
        includeZaloConfigs: document.getElementById("domain-migration-include-zalo")?.checked !== false,
        onBatch: (p) => appendLog(`${p.table || "?"} — đã tải ${p.total} dòng...`)
      });
      logPre.textContent = formatDomainMigrationReportText(lastReport);
      thongbao(ti(`Quét xong: ${lastReport.totalPending} mục cần migrate`, `Scan done: ${lastReport.totalPending} items to migrate`, `扫描完成：${lastReport.totalPending} 项待迁移`));
    } catch (e) {
      appendLog(ti("Lỗi: ", "Error: ", "错误：") + (e.message || e));
      canhbao(e.message || String(e));
    } finally {
      scanBtn.disabled = dryRunBtn.disabled = migrateBtn.disabled = false;
    }
  };

  dryRunBtn.onclick = async () => {
    if (!lastReport || !lastReport.totalPending) {
      return canhbao(ti("Chạy «Quét dữ liệu cũ» trước", "Run «Scan legacy data» first", "请先运行«扫描旧数据»"));
    }
    dryRunBtn.disabled = migrateBtn.disabled = true;
    try {
      ...
      appendLog(ti(
        `DRY-RUN xong — detail:${stats.detail}, services:${stats.services}, zalo:${stats.zalo}`,
        `DRY-RUN done — detail:${stats.detail}, services:${stats.services}, zalo:${stats.zalo}`,
        `试运行完成 — detail:${stats.detail}, services:${stats.services}, zalo:${stats.zalo}`
      ));
    } catch (e) {
      appendLog(ti("Lỗi dry-run: ", "Dry-run error: ", "试运行错误：") + (e.message || e));
    } finally {
      dryRunBtn.disabled = migrateBtn.disabled = false;
    }
  };

  migrateBtn.onclick = async () => {
    if (!lastReport || !lastReport.totalPending) {
      return canhbao(ti("Không có dữ liệu cần migrate — quét lại", "Nothing to migrate — scan again", "无需迁移 — 请重新扫描"));
    }
    const gs = getGlobalSettings();
    const ok = confirm(ti(
      `Migrate ${lastReport.totalPending} mục sang domain:\n${lastReport.targetDomain}\n\n(app: ${DOMAIN_OPTIONS[gs.domainKey]?.app_id})\n\nTiếp tục?`,
      `Migrate ${lastReport.totalPending} items to:\n${lastReport.targetDomain}\n\n(app: ${DOMAIN_OPTIONS[gs.domainKey]?.app_id})\n\nContinue?`,
      `将 ${lastReport.totalPending} 项迁移到：\n${lastReport.targetDomain}\n\n(app: ${DOMAIN_OPTIONS[gs.domainKey]?.app_id})\n\n继续？`
    ));
    if (!ok) return;
    ...
    try {
      ...
      const errLine = stats.errors.length
        ? `\n⚠️ Lỗi: ${stats.errors.length} (xem console)`
        : "";
      appendLog(ti(
        `✅ Hoàn tất — detail:${stats.detail}, services:${stats.services}, zalo:${stats.zalo}${errLine}`,
        `✅ Done — detail:${stats.detail}, services:${stats.services}, zalo:${stats.zalo}${errLine}`,
        `✅ 完成 — detail:${stats.detail}, services:${stats.services}, zalo:${stats.zalo}${errLine}`
      ));
      if (stats.errors.length) console.warn("[DomainMigration] errors:", stats.errors);
      thongbao(ti("Migrate domain hoàn tất", "Domain migration completed", "域名迁移完成"));
      lastReport = await scanDomainMigrationInventory(gs.domainKey, {
        scopeServiceType: scopeOnlyCurrent.checked ? (gs.isLmkt ? gs.project : gs.industry) : "",
        includeDetail: document.getElementById("domain-migration-include-detail")?.checked !== false,
        includeServices: document.getElementById("domain-migration-include-services")?.checked !== false,
        includeZaloConfigs: document.getElementById("domain-migration-include-zalo")?.checked !== false
      });
      ...
      appendLog(ti("Lỗi migrate: ", "Migration error: ", "迁移错误：") + (e.message || e));
      canhbao(e.message || String(e));
    } finally {
      migrateBtn.disabled = scanBtn.disabled = dryRunBtn.disabled = false;
    }
  };

  const domainSelect = () => document.getElementById("global-domain-select");
  if (domainSelect()) {
    domainSelect().addEventListener("change", refreshTargetLabel);
  }
  refreshTargetLabel();

  btnRow.append(scanBtn, dryRunBtn, migrateBtn);
  panel.append(title, hint, targetPreview, scopeRow, checkWrap, btnRow, logPre);
  return panel;
}

function articleBelongsToDomainKey(article, domainKey) {
  if (!article) return false;
  return resolveDomainKeyFromValue(article.domain) === domainKey;
}
...
function formatDuplicateDomainReportText(report) {
  const lines = [
    `🌐 Domain key: ${report.domainKey} → ${report.canonicalDomain}`,
    report.scopeServiceType ? `🔎 Lọc service_type: ${report.scopeServiceType}` : "🔎 Phạm vi: mọi lĩnh vực/dự án trong domain",
    `📰 Tổng bài active (mọi biến thể domain): ${report.totalArticles}`,
    "",
    "📋 Biến thể domain trong DB:",
    ...Object.entries(report.domainVariants || {}).map(([d, n]) => `   • ${d}: ${n} bài`),
    "",
    "📂 Số bài theo service_type:",
    ...Object.entries(report.byServiceType || {}).map(([st, n]) => `   • ${st}: ${n} bài`),
    "",
    `🔁 Nhóm trùng: ${report.duplicateGroupCount} (sẽ xóa ~${report.articlesToRemove} bài cũ, giữ bài mới nhất/nhóm)`,
  ];
  if (report.duplicateGroupCount > 0) {
    lines.push("", "📊 Nhóm trùng theo service_type:");
    Object.entries(report.duplicateGroupsByServiceType || {}).forEach(([st, n]) => {
      lines.push(`   • ${st}: ${n} nhóm`);
    });
    const samples = (report.duplicateGroups || []).slice(0, 8);
    if (samples.length) {
      lines.push("", "📝 Mẫu (tối đa 8 nhóm):");
      samples.forEach((g, i) => {
        const keep = g.keepArticle?.title || g.keepArticle?.slug || "?";
        lines.push(`   ${i + 1}. Giữ: ${String(keep).slice(0, 60)} — xóa ${g.removeArticles?.length || 0} bài`);
      });
    }
  }
  return lines.join("\n");
...
async function scanDuplicatesByDomain(domainKey, options = {}) {
  const ctx = options.ctx || resolveContext();
  ctx.app_id = DOMAIN_OPTIONS[domainKey]?.app_id || ctx.app_id;
  ...
  articles.forEach((a) => {
    const st = String(a.service_type || "(none)");
    byServiceType[st] = (byServiceType[st] || 0) + 1;
    const d = String(a.domain || "(trống)").trim();
    domainVariants[d] = (domainVariants[d] || 0) + 1;
  });

  const findOpts = {
    titleThreshold: options.titleThreshold ?? 0.75,
    contentHashThreshold: options.contentHashThreshold !== false,
    imageThreshold: options.imageThreshold ?? 0.5,
    thumbnailThreshold: options.thumbnailThreshold ?? 0.85
  };

  const duplicateGroups = articles.length >= 2
    ? await findDuplicateArticles(articles, findOpts)
    : [];

  const articlesToRemove = duplicateGroups.reduce(
    (sum, g) => sum + (g.removeArticles?.length || 0),
    0
  );
  const duplicateGroupsByServiceType = {};
  duplicateGroups.forEach((g) => {
    const st = String(g.keepArticle?.service_type || "(none)");
    duplicateGroupsByServiceType[st] = (duplicateGroupsByServiceType[st] || 0) + 1;
  });

  return {
    domainKey,
    canonicalDomain: canonicalDomainForKey(domainKey),
    scopeServiceType: options.scopeServiceType || "",
    ...
    byServiceType,
    domainVariants,
    duplicateGroupsByServiceType,
    duplicateGroups
  };
}

async function cleanupDuplicatesByDomain(domainKey, options = {}) {
  const scan = options.preloadedScan || await scanDuplicatesByDomain(domainKey, options);

  if (scan.totalArticles < 2) {
    return {
      success: false,
      message: ti(
        `❌ Không đủ bài (${scan.totalArticles}) để kiểm tra trùng`,
        `❌ Not enough articles (${scan.totalArticles}) to check duplicates`,
        `❌ 文章不足（${scan.totalArticles}）无法查重`
      ),
      duplicateCount: 0,
      cleanedCount: 0,
      scan
  ...
  if (!scan.duplicateGroupCount) {
    return {
      success: true,
      message: ti("✅ Không tìm thấy bài trùng trong domain", "✅ No duplicates in domain", "✅ 该域名下无重复文章"),
      duplicateCount: 0,
      cleanedCount: 0,
      scan
    };
  ...
  if (options.dryRun) {
    return {
      ...
      dryRun: true,
      message: ti(
        `[DRY-RUN] Sẽ xóa ${scan.articlesToRemove} bài / ${scan.duplicateGroupCount} nhóm`,
        `[DRY-RUN] Would delete ${scan.articlesToRemove} posts in ${scan.duplicateGroupCount} groups`,
        `[DRY-RUN] 将删除 ${scan.articlesToRemove} 篇 / ${scan.duplicateGroupCount} 组`
      ),
      duplicateCount: scan.duplicateGroupCount,
      cleanedCount: 0,
      wouldDelete: scan.articlesToRemove,
      scan
    };
  }

  const ctx = options.ctx || resolveContext();
  const cleanupResult = await cleanupDuplicateArticles(scan.duplicateGroups, ctx);
  return {
    ...
    scan,
    details: scan.duplicateGroups.map((g) => ({
      kept: g.keepArticle?.title,
      removed: (g.removeArticles || []).map((a) => a.title),
      service_type: g.keepArticle?.service_type
    }))
  };
}

function ensureDomainDuplicateCleanupPanel(theme) {
  if (!theme) theme = getThemeTokens();
  let panel = document.getElementById("domain-duplicate-cleanup-panel");
  if (panel) return panel;

  panel = document.createElement("div");
  panel.id = "domain-duplicate-cleanup-panel";
  panel.style.cssText = `margin-top:14px;padding:12px;border:1px solid ${theme.border};border-radius:8px;background:${theme.surface || theme.bg};`;

  const title = document.createElement("div");
  title.style.cssText = `font-weight:700;margin-bottom:6px;color:${theme.text};font-size:13px;`;
  title.textContent = ti(
    "🧹 Dọn tin trùng theo domain (toàn bộ)",
    "🧹 Cleanup duplicates by domain (full)",
    "🧹 按域名清理重复文章（全部）"
  );
  ...
  const targetPreview = document.createElement("div");
  targetPreview.id = "domain-dedup-target";
  targetPreview.style.cssText = `font-size:12px;margin-bottom:8px;color:${theme.text};`;
  ...
  const scopeOnlyCurrent = document.createElement("input");
  scopeOnlyCurrent.type = "checkbox";
  scopeOnlyCurrent.id = "domain-dedup-scope-current";
  scopeOnlyCurrent.checked = false;
  scopeRow.appendChild(scopeOnlyCurrent);
  scopeRow.appendChild(document.createTextNode(ti(
    "Chỉ lĩnh vực/dự án đang chọn (giống nút Zalo)",
    "Only current industry/project (like Zalo button)",
    "仅当前行业/项目（同 Zalo 按钮）"
  )));
  ...
  const scanBtn = createButton(ti("🔍 Quét trùng domain", "🔍 Scan domain duplicates", "🔍 扫描域名重复"), "#595959");
  const dryRunBtn = createButton(ti("🧪 Chạy thử (không xóa)", "🧪 Dry run (no delete)", "🧪 试运行（不删除）"), "#fa8c16");
  const cleanupBtn = createButton(ti("🧹 Dọn trùng & xóa bài cũ", "🧹 Cleanup & delete old", "🧹 清理并删除旧文"), "#ff7a45");

  const logPre = document.createElement("pre");
  logPre.id = "domain-dedup-log";
  logPre.style.cssText = `margin:0;padding:10px;max-height:260px;overflow:auto;font-size:11px;line-height:1.45;white-space:pre-wrap;border:1px solid ${theme.border};border-radius:6px;background:${theme.inputBg};color:${theme.text};`;
  ...
  const refreshTarget = () => {
    const gs = getGlobalSettings();
    targetPreview.textContent = ti(
      `Domain: ${canonicalDomainForKey(gs.domainKey)} — app ${DOMAIN_OPTIONS[gs.domainKey]?.app_id || "?"}`,
      `Domain: ${canonicalDomainForKey(gs.domainKey)} — app ${DOMAIN_OPTIONS[gs.domainKey]?.app_id || "?"}`,
      `域名：${canonicalDomainForKey(gs.domainKey)} — app ${DOMAIN_OPTIONS[gs.domainKey]?.app_id || "?"}`
    );
  };
  ...
  const getScopeServiceType = () => {
    const gs = getGlobalSettings();
    if (!scopeOnlyCurrent.checked) return "";
    return gs.isLmkt ? gs.project : gs.industry;
  };

  scanBtn.onclick = async () => {
    refreshTarget();
    const gs = getGlobalSettings();
    scanBtn.disabled = dryRunBtn.disabled = cleanupBtn.disabled = true;
    logPre.textContent = "";
    appendLog(ti("Đang tải bài và phân tích trùng (có thể vài phút)...", "Loading posts and detecting duplicates...", "正在加载并分析重复..."));
    try {
      const ctx = resolveContext();
      ctx.app_id = DOMAIN_OPTIONS[gs.domainKey]?.app_id || ctx.app_id;
      lastScan = await scanDuplicatesByDomain(gs.domainKey, {
        ctx,
        scopeServiceType: getScopeServiceType(),
        onBatch: (p) => appendLog(ti(`Đã tải ${p.total} dòng...`, `Loaded ${p.total} rows...`, `已加载 ${p.total} 行...`))
      });
      logPre.textContent = formatDuplicateDomainReportText(lastScan);
      thongbao(ti(
        `Quét xong: ${lastScan.duplicateGroupCount} nhóm, ~${lastScan.articlesToRemove} bài sẽ xóa`,
        `Scan done: ${lastScan.duplicateGroupCount} groups, ~${lastScan.articlesToRemove} to delete`,
        `扫描完成：${lastScan.duplicateGroupCount} 组，约删除 ${lastScan.articlesToRemove} 篇`
      ));
    } catch (e) {
      appendLog(ti("Lỗi: ", "Error: ", "错误：") + (e.message || e));
      canhbao(e.message || String(e));
    } finally {
      scanBtn.disabled = dryRunBtn.disabled = cleanupBtn.disabled = false;
    }
  };

  dryRunBtn.onclick = async () => {
    if (!lastScan?.duplicateGroupCount) {
      return canhbao(ti("Chạy «Quét trùng domain» trước", "Run «Scan domain duplicates» first", "请先「扫描域名重复」"));
    }
    dryRunBtn.disabled = cleanupBtn.disabled = true;
    try {
      const gs = getGlobalSettings();
      const ctx = resolveContext();
      ...
      dryRunBtn.disabled = cleanupBtn.disabled = false;
    }
  };

  cleanupBtn.onclick = async () => {
    if (!lastScan?.duplicateGroupCount) {
      return canhbao(ti("Không có nhóm trùng — quét lại", "No duplicate groups — scan again", "无重复组 — 请重新扫描"));
    }
    const gs = getGlobalSettings();
    const ok = confirm(ti(
      `Xóa ~${lastScan.articlesToRemove} bài cũ trong ${lastScan.duplicateGroupCount} nhóm trùng?\n\nDomain: ${lastScan.canonicalDomain}\n\nHành động không hoàn tác.`,
      `Delete ~${lastScan.articlesToRemove} old posts in ${lastScan.duplicateGroupCount} duplicate groups?\n\nDomain: ${lastScan.canonicalDomain}\n\nCannot undo.`,
      `删除约 ${lastScan.articlesToRemove} 篇旧文（${lastScan.duplicateGroupCount} 组）？\n\n域名：${lastScan.canonicalDomain}\n\n不可撤销。`
    ));
    if (!ok) return;

    cleanupBtn.disabled = scanBtn.disabled = dryRunBtn.disabled = true;
    try {
      const ctx = resolveContext();
      appendLog(ti("Đang xóa bài trùng...", "Deleting duplicates...", "正在删除重复文章..."));
      const result = await cleanupDuplicatesByDomain(gs.domainKey, {
        ctx,
        scopeServiceType: getScopeServiceType(),
        dryRun: false,
      ...
      cleanupBtn.disabled = scanBtn.disabled = dryRunBtn.disabled = false;
    }
  };

  const domainSelect = () => document.getElementById("global-domain-select");
  if (domainSelect()) domainSelect().addEventListener("change", refreshTarget);
  refreshTarget();

  btnRow.append(scanBtn, dryRunBtn, cleanupBtn);
  panel.append(title, hint, targetPreview, scopeRow, btnRow, logPre);
  return panel;
}
...
function ensureGlobalSettingsPanel() {
  ...
  const domainMigrationSection = ensureDomainMigrationPanel(theme);
  const domainDuplicateSection = ensureDomainDuplicateCleanupPanel(theme);

  // Append title and container to wrapper
  wrapper.append(title, settingsContainer, domainMigrationSection, domainDuplicateSection, facebookCommonSection);
...
async function updateDetailDomainByServiceType(domainKey, serviceType) {
  try {
    const report = await scanDomainMigrationInventory(domainKey, {
      scopeServiceType: serviceType,
      includeServices: false,
      includeZaloConfigs: false
    });
    const stats = await executeDomainMigration(report, { dryRun: false });
    const message = `✅ Đã cập nhật domain cho ${stats.detail} tin chi tiết`;
    if (window.showNotification) {
      window.showNotification({ type: 'success', message, duration: 3 });
    } else {
      alert(message);
