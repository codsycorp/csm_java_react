(function autoKqxsReactAntdSelfContained() {
  ...
  var UI_TEXT = {
    vi: {
      ...
      lgKttBtnDo3NamBacDao: "Lấy KQ 3 Nam đảo",
      lgKttBtnDo3NamBacCD: "Lấy KQ 3 Nam CĐ",
      lgKttBtnDoBacNamHomSau: "Lấy KQ Bắc",
      lgKttBtnDoBacNamHomSauDao: "Lấy KQ Bắc đảo",
      lgKttBtnDoBacNamHomSauCD: "Lấy KQ Bắc CĐ",
  ...
  function normalizeLegacyLoaiTimFieldToken(token, heThong, maQuery) {
    ...
    var isGet3Dd = mq === "SP_GET3_DD" || mq === "SP_GET3_DD3";
    var isGet2Dd = mq === "SP_GET2_DD" || mq === "SP_GET2_DD3";
    var isGet3Bl = mq === "SP_GET3_BL" || mq === "SP_GET3_BL3";
    var isGet2Bl = mq === "SP_GET2_BL" || mq === "SP_GET2_BL3";
    ...
    if (isGet3Bl && role === "B" && tail === "dau") return "B_so5";
    if (isGet2Bl && role === "B" && tail === "so5") return "B_dau";

    if (he === 3 && isGet3Bl && (role === "D" || role === "P" || role === "T") && tail === "dau") {
      return role + "_so2";
    }
  ...
  function parseLegacyLoaiTimFieldPart(rawLoai, heThong) {
    return parseLegacyLoaiTimMeta(rawLoai, heThong).fieldPart;
  }
  ...
  function getLegacyQueryBaseFieldList(queryValue, heThong) {
    ...
    var baseFields = [];
      ...
      else if (maQuery === "SP_GET3_DD" || maQuery === "SP_GET3_DD3") {
        baseFields = ["D_so2", "D_duoi", "P_so2", "P_duoi", "T_so2", "T_duoi", "B_so5", "B_so6", "B_so7", "B_duoi"];
      }
      else if (maQuery === "SP_GET3_BL" || maQuery === "SP_GET3_BL3") {
  ...
  function getLegacyQueryFieldList(queryValue, heThong) {
    var he = Number(heThong || 2) === 3 ? 3 : 2;
    var baseFields = getLegacyQueryBaseFieldList(queryValue, he);

      var fieldTokens = String(parseLegacyLoaiTimFieldPart(queryValue || "", he) || "")
        .split("-")
  ...
  function KQXSApp() {
    ...
    var _ax7 = useState({}), legacyKttDo3NamBacDaoHitMap = _ax7[0], setLegacyKttDo3NamBacDaoHitMap = _ax7[1];
    var _ax8 = useState(false), legacyKttDo3NamBacDaoActive = _ax8[0], setLegacyKttDo3NamBacDaoActive = _ax8[1];
    var _ax9 = useState({}), legacyKttDo3NamBacCDHitMap = _ax9[0], setLegacyKttDo3NamBacCDHitMap = _ax9[1];
    var _ax10 = useState(false), legacyKttDo3NamBacCDActive = _ax10[0], setLegacyKttDo3NamBacCDActive = _ax10[1];
    var _ax11 = useState({}), legacyKttDoBacNamHomSauDaoHitMap = _ax11[0], setLegacyKttDoBacNamHomSauDaoHitMap = _ax11[1];
    var _ax12 = useState(false), legacyKttDoBacNamHomSauDaoActive = _ax12[0], setLegacyKttDoBacNamHomSauDaoActive = _ax12[1];
    var _ax13 = useState({}), legacyKttDoBacNamHomSauCDHitMap = _ax13[0], setLegacyKttDoBacNamHomSauCDHitMap = _ax13[1];
    var _ax14 = useState(false), legacyKttDoBacNamHomSauCDActive = _ax14[0], setLegacyKttDoBacNamHomSauCDActive = _ax14[1];
    ...
    function resolveLegacyTongHopFieldLocList(queryValue) {
      return parseLegacyLoaiTimFieldPart(queryValue || "D_-P_-T_-B_", legacyHeThong)
        .split("-")
    ...
    async function fetchLegacyTongHopRowsFromApi(opts) {
      ...
      // Dữ liệu có thể thiếu một phần nhưng vẫn đủ để tính ra kết quả.
      // Chỉ báo lỗi khi thiếu dữ liệu và hoàn toàn không dựng được metrics.
    ...
    function getLegacyThFilterConfig() {
      ...
      var c8Tong29MinTop = parseInt(legacyThAutoC8Tong29Min, 10) || 0;
      return {
        c1NgayTop: c1NgayTop,
        c1KyTop: c1KyTop,
        c2NgayCX: c2NgayCX,
    ...
    function filterLegacyThRowsWithConfig(rows, filterCfg) {
      ...
      if (cfg.c8Tong29MinTop > 0) {
        sourceRows.slice().sort(function (a, b) {
          return (Number(a.lanTongNgaySinh) || 0) - (Number(b.lanTongNgaySinh) || 0);
        }).slice(0, cfg.c8Tong29MinTop).forEach(function (row) {
          c8Tong29MinMap[row.key] = 1;
        });
      }
    ...
    function buildLegacyKttDo3NamBacHitMap(rows, heThong, queryValue, variant) {
      var out = {};
      var list = Array.isArray(rows) ? rows : [];
      var mode = String(variant || "main").toLowerCase();
      for (var i = 0; i < list.length; i += 1) {
        var row = list[i] || {};
        ...
        for (var ni = 0; ni < namFields.length; ni += 1) {
          ...
          if (!namToken) continue;
          var namCandidates = getLegacyKttVariantTokenCandidates(namToken, mode, "left");
          namCandidates.forEach(function (candidate) {
            if (!namTokenToFields[candidate]) namTokenToFields[candidate] = {};
            namTokenToFields[candidate][namField] = true;
          });
        }
        if (!Object.keys(namTokenToFields).length) continue;
        var dayHits = {};
        var anyHit = false;
        for (var bi = 0; bi < bacFields.length; bi += 1) {
          var bacField = bacFields[bi];
          var bacToken = getLegacyKttFieldToken(row[bacField], heThong);
          if (!bacToken) continue;
          var bacCandidates = getLegacyKttVariantTokenCandidates(bacToken, mode, "right");
          var matchedToken = "";
          for (var bci = 0; bci < bacCandidates.length; bci += 1) {
            if (namTokenToFields[bacCandidates[bci]]) {
              matchedToken = bacCandidates[bci];
              break;
            }
          }
          if (!matchedToken) continue;
          dayHits[bacField] = true;
          var matchedNamFields = namTokenToFields[matchedToken] || {};
    ...
    function buildLegacyKttDoBacNamHomSauHitMap(rows, heThong, queryValue, variant) {
      var out = {};
      var list = Array.isArray(rows) ? rows : [];
      var rowByYmd = {};
      var mode = normalizeLegacyKttVariantMode(variant);
      ...
      function getCellDisplayTokenList(rawVal, fieldName, heValue) {
        return getLegacyKttCrossDayTokenList(rawVal, fieldName, heValue);
      }
      ...
      for (var i = 0; i < list.length; i += 1) {
        ...
        for (var nfi = 0; nfi < northFieldOrder.length; nfi += 1) {
          ...
          for (var nti = 0; nti < northTokenList.length; nti += 1) {
            ...
            var northCandidates = getLegacyKttBacNamVariantCandidates(northToken, mode);
            northCandidates.forEach(function (candidateToken) {
              if (!northTokenToFields[candidateToken]) northTokenToFields[candidateToken] = {};
              northTokenToFields[candidateToken][northField] = true;
            });
          }
        }
        ...
        for (var sidx = 0; sidx < southFieldOrder.length; sidx += 1) {
          ...
          for (var sti = 0; sti < southTokenList.length; sti += 1) {
            var southToken = String(southTokenList[sti] || "");
            if (!southToken || !northTokenToFields[southToken]) continue;
            if (!matchedTokenSet[southToken]) {
              matchedTokenSet[southToken] = true;
              matchedTokens.push(southToken);
            }
          }

          if (!matchedTokens.length) continue;

          var hitToken = matchedTokens[0];
          southDayHits[southField] = hitToken;
          southDayHits[normalizeLegacyKttDisplayField(southField, heThong)] = hitToken;

          matchedTokens.forEach(function (matchedToken) {
            var linkedNorthFields = northTokenToFields[matchedToken] || {};
            Object.keys(linkedNorthFields).forEach(function (nf) {
              northDayHits[nf] = matchedToken;
              northDayHits[normalizeLegacyKttDisplayField(nf, heThong)] = matchedToken;
            });
          });
        ...
        if (anyHit) {
          if (Object.keys(northDayHits).length) {
            out[northYmd] = Object.assign({}, out[northYmd] || {}, northDayHits);
          }
          if (Object.keys(southDayHits).length) {
            out[southYmd] = Object.assign({}, out[southYmd] || {}, southDayHits);
          }
        }
      }
    ...
    function getLegacySoLauRaTokenFields(queryValue, heThong, dataRows, options) {
      var cfg = options || {};
      var queryFields = cfg.fullFields ? getLegacyAllNormalizedFieldKeys() : getLegacyQueryFieldList(queryValue, heThong);
      ...
      var fieldParts = cfg.fullFields ? [] : parseLegacyLoaiTimFieldPart(queryValue, heThong).split("-").map(function (part) {
    ...
    function buildLegacySoLauRaWeekRows(opts) {
      ...
      var tokenFields = getLegacySoLauRaTokenFields(cfg.queryValue, Number(cfg.heThong || 2) === 3 ? 3 : 2, baseRows, { fullFields: !!cfg.fullTokenFields });
    ...
    function buildLegacyNamBacSummary(opts) {
      ...
      var expectedCols = dedupFields(
        getLegacyQueryFieldList(cfg.queryValue || queryBL, heThong).concat(getLegacyAllNormalizedFieldKeys())
      );
      var actualMap = {};
      ...
      var DD3D_FIELDS = heThong === 3
        ? { D_dau: true, D_so2: true, D_duoi: true, P_dau: true, P_so2: true, P_duoi: true, T_dau: true, T_so2: true, T_duoi: true, B_dau: true, B_so2: true, B_so3: true, B_so5: true, B_so6: true, B_so7: true, B_duoi: true }
      ...
      var DDMB_FIELDS = heThong === 3
        ? { B_dau: true, B_so2: true, B_so3: true, B_so5: true, B_so6: true, B_so7: true, B_duoi: true }
        : { B_dau: true, B_so2: true, B_so3: true, B_so4: true, B_duoi: true };
      var DAUB_FIELDS = heThong === 3
        ? { B_dau: true, B_so2: true, B_so3: true, B_so5: true, B_so6: true, B_so7: true }
        : { B_dau: true, B_so2: true, B_so3: true, B_so4: true };
      var DD2D_FIELDS = heThong === 3
        ? { D_dau: true, D_so2: true, D_duoi: true, P_dau: true, P_so2: true, P_duoi: true }
        : { D_dau: true, D_duoi: true, P_dau: true, P_duoi: true };
      var DD3MN_FIELDS = heThong === 3
        ? { D_dau: true, D_so2: true, D_duoi: true, P_dau: true, P_so2: true, P_duoi: true, T_dau: true, T_so2: true, T_duoi: true }
        : { D_dau: true, D_duoi: true, P_dau: true, P_duoi: true, T_dau: true, T_duoi: true };
      var DDNDB_FIELDS = heThong === 3
        ? { D_dau: true, D_so2: true, D_duoi: true, P_dau: true, P_so2: true, P_duoi: true, B_dau: true, B_so2: true, B_so3: true, B_so5: true, B_so6: true, B_so7: true }
      ...
      var DDNDB3_FIELDS = heThong === 3
        ? { D_dau: true, D_so2: true, D_duoi: true, P_dau: true, P_so2: true, P_duoi: true, T_dau: true, T_so2: true, T_duoi: true, B_dau: true, B_so2: true, B_so3: true, B_so5: true, B_so6: true, B_so7: true }
        : { D_dau: true, D_duoi: true, P_dau: true, P_duoi: true, T_dau: true, T_duoi: true, B_dau: true, B_so2: true, B_so3: true, B_so4: true };
      var DDC_FIELDS = heThong === 3 ? { D_dau: true, D_so2: true, D_duoi: true } : { D_dau: true, D_duoi: true };
      var DAUCP_FIELDS = heThong === 3 ? { D_dau: true, D_so2: true, P_dau: true, P_so2: true } : { D_dau: true, P_dau: true };
      var DDP_FIELDS = heThong === 3 ? { P_dau: true, P_so2: true, P_duoi: true } : { P_dau: true, P_duoi: true };
      var DAI3_FIELDS = heThong === 3 ? { T_dau: true, T_so2: true, T_duoi: true } : { T_dau: true, T_duoi: true };
      var DUOICP_FIELDS = { D_duoi: true, P_duoi: true };
      var DD3NB_FIELDS = heThong === 3
        ? { D_dau: true, D_so2: true, D_duoi: true, P_dau: true, P_so2: true, P_duoi: true, T_dau: true, T_so2: true, T_duoi: true, B_dau: true, B_so2: true, B_so3: true, B_so5: true, B_so6: true, B_so7: true, B_duoi: true }
        : { D_dau: true, D_duoi: true, P_dau: true, P_duoi: true, T_dau: true, T_duoi: true, B_dau: true, B_so2: true, B_so3: true, B_so4: true, B_duoi: true };

      function isLegacyNamBacField(fieldName, station, role) {
        var name = String(fieldName || "").trim();
        if (!name) return false;
        var headFields = heThong === 3
          ? {
              D: { D_dau: true, D_so2: true },
              P: { P_dau: true, P_so2: true },
              T: { T_dau: true, T_so2: true },
              B: { B_dau: true, B_so2: true, B_so3: true, B_so5: true, B_so6: true, B_so7: true }
            }
          : {
              D: { D_dau: true },
              P: { P_dau: true },
              T: { T_dau: true },
              B: { B_dau: true, B_so2: true, B_so3: true, B_so4: true }
            };
        var tailFields = {
          D: { D_duoi: true },
          P: { P_duoi: true },
        ...
        if (role === "dau") {
          return !!(headFields[station] && headFields[station][name]);
        }
        if (role === "duoi") {
          return !!(tailFields[station] && tailFields[station][name]);
        }
        return false;
      }
      ...
      for (var ci = 0; ci < colA.length; ci += 1) {
        ...
        fieldProfiles[fieldName] = {
          ...
          daub: isLegacyNamBacField(fieldName, "B", "dau"),
          ...
          dauc: isLegacyNamBacField(fieldName, "D", "dau"),
          daup: isLegacyNamBacField(fieldName, "P", "dau"),
          duoic: isLegacyNamBacField(fieldName, "D", "duoi"),
          duoip: isLegacyNamBacField(fieldName, "P", "duoi"),
          daut3: isLegacyNamBacField(fieldName, "T", "dau"),
          duoit3: isLegacyNamBacField(fieldName, "T", "duoi"),
          duoib: isLegacyNamBacField(fieldName, "B", "duoi"),
          ...
          dauc_duoip: isLegacyNamBacField(fieldName, "D", "dau") || isLegacyNamBacField(fieldName, "P", "duoi"),
          daup_duoic: isLegacyNamBacField(fieldName, "P", "dau") || isLegacyNamBacField(fieldName, "D", "duoi")
        };
      }
      ...
      function makeNamBacGapTracker(result) {
        var done = {};
        function mark(key, hit) {
          if (done[key]) return;
          if (hit) {
            done[key] = true;
            return;
      ...
      var trackers = states.map(function (st) {
        return makeNamBacGapTracker(st.result);
      });
      ...
      for (var r = 0; r < rows.length; r += 1) {
        ...
        for (var s = 0; s < states.length; s += 1) {
          ...
          var dayBlNb = false, dayBlMn = false, dayBlDc = false, dayBlDp = false;
          var dayBlD3 = false, dayBlMb = false, dayBl3Mn = false, dayBlNb3 = false;
          ...
          for (var c = 0; c < colA.length; c += 1) {
            ...
            if (fp.dauc) dayDauC = true;
            if (fp.daup) dayDauP = true;
            if (fp.daut3) dayDauT = true;
            if (fp.daub) dayDauB = true;
            if (fp.duoic) dayDuoiC = true;
            if (fp.duoip) dayDuoiP = true;
            if (fp.duoit3) dayDuoiT = true;
            if (fp.duoib) dayDuoiB = true;
            if (fp.blnb) dayBlNb = true;
            if (fp.blmn) dayBlMn = true;
            if (fp.bldc) dayBlDc = true;
            if (fp.bldp) dayBlDp = true;
            if (fp.bld3) dayBlD3 = true;
            if (fp.blmb) dayBlMb = true;
            if (fp.bl3mn) dayBl3Mn = true;
            if (fp.blnb3) dayBlNb3 = true;
          }
          ...
          var dayHits = {
            ...
            duoiT: dayDuoiT,
            duoiB: dayDuoiB,
            ddC: dayDauC || dayDuoiC,
            ddP: dayDauP || dayDuoiP,
            ddT: dayDauT || dayDuoiT,
            ddB: dayDauB || dayDuoiB,
            blC: dayBlDc,
            blP: dayBlDp,
            blT: dayBlD3,
            blB: dayBlMb
          ...
          function hitAny(keys) {
            for (var hi = 0; hi < keys.length; hi += 1) {
              if (dayHits[keys[hi]]) return true;
            }
            return false;
          }

          markGap("DAU_C", hitAny(["dauC"]));
          markGap("DAU_P", hitAny(["dauP"]));
          markGap("DAU_T", hitAny(["dauT"]));
          markGap("DAU_B", hitAny(["dauB"]));
          markGap("DAU_CP", hitAny(["dauC", "dauP"]));
          markGap("DUOI_C", hitAny(["duoiC"]));
          markGap("DUOI_P", hitAny(["duoiP"]));
          markGap("DUOI_T", hitAny(["duoiT"]));
          markGap("DUOI_B", hitAny(["duoiB"]));
          markGap("DUOI_CP", hitAny(["duoiC", "duoiP"]));
          markGap("DAU_C_DUOI_P", hitAny(["dauC", "duoiP"]));
          markGap("DAU_P_DUOI_C", hitAny(["dauP", "duoiC"]));
          markGap("DD_C", hitAny(["ddC"]));
          markGap("DD_P", hitAny(["ddP"]));
          markGap("DD_T", hitAny(["ddT"]));
          markGap("DD_B", hitAny(["ddB"]));
          markGap("DD_2_NAM", hitAny(["ddC", "ddP"]));
          markGap("DD_3_NAM", hitAny(["ddC", "ddP", "ddT"]));
          markGap("DD_3_NB", hitAny(["ddC", "ddP", "ddB"]));
          markGap("DD_2_NAM_B_DAU", hitAny(["ddC", "ddP", "dauB"]));
          markGap("DD_3_NAM_B_DAU", hitAny(["ddC", "ddP", "ddT", "dauB"]));
          markGap("DD_3_NB_DAU", hitAny(["ddC", "ddP", "ddB"]));
          markGap("DD_4_NHOM", hitAny(["ddC", "ddP", "ddT", "ddB"]));
          markGap("BL_C", hitAny(["blC"]));
          markGap("BL_P", hitAny(["blP"]));
          markGap("BL_T", hitAny(["blT"]));
          markGap("BL_B", hitAny(["blB"]));
          markGap("BL_2_NAM", hitAny(["blC", "blP"]));
          markGap("BL_3_NAM", hitAny(["blC", "blP", "blT"]));
          markGap("BL_3_NB", hitAny(["blC", "blP", "blB"]));
          markGap("BL_4_NHOM", hitAny(["blC", "blP", "blT", "blB"]));
        }
      }
    ...
    function buildTongHopMetrics(opts) {
      ...
      var summaryBaseFields = getLegacyQueryBaseFieldList(queryValue, heThong);
      var actualFieldMap = {};
      rows.forEach(function (row) {
        Object.keys(row || {}).forEach(function (key) {
          var name = String(key || "").trim();
          if (/^[DPTB]_(dau|duoi|so\d+)$/i.test(name)) actualFieldMap[name] = true;
        });
      });
      var summaryFields = summaryBaseFields.filter(function (name) {
        return !!actualFieldMap[String(name || "")];
      });
      if (!summaryFields.length) summaryFields = summaryBaseFields.slice();
      ...
      for (var k = 0; k < cachList.length; k++) {
        ...
        for (var r = 0; r < rows.length; r++) {
          ...
          // Sub-group counts widen only inside the same query family/system.
          // DD stays DD, BL stays BL; choosing P_dau still needs T/B DD fields for D3/MB/3NB.
          for (var c2 = 0; c2 < summaryFields.length; c2++) {
            var fn2 = summaryFields[c2];
    ...
    function resetLegacyKttVariantStates() {
      setLegacyKttDo3NamBacHitMap({});
      setLegacyKttDo3NamBacActive(false);
      setLegacyKttDo3NamBacDaoHitMap({});
      setLegacyKttDo3NamBacDaoActive(false);
      setLegacyKttDo3NamBacCDHitMap({});
      setLegacyKttDo3NamBacCDActive(false);
      setLegacyKttDoBacNamHomSauHitMap({});
      setLegacyKttDoBacNamHomSauActive(false);
      setLegacyKttDoBacNamHomSauDaoHitMap({});
      setLegacyKttDoBacNamHomSauDaoActive(false);
      setLegacyKttDoBacNamHomSauCDHitMap({});
      setLegacyKttDoBacNamHomSauCDActive(false);
    }
    ...
    async function runLegacyKttDo3NamBac(silent, variant) {
      var mode = normalizeLegacyKttVariantMode(variant);
      setLoading(true);
      setProgress(15);
      try {
        var payload = await fetchLegacyKttViewModelPayload();
        if (!payload) return;
        var hitMap = buildLegacyKttDo3NamBacHitMap(payload.rows, legacyHeThong, payload.queryValue, mode);
        applyLegacyKttViewPayload(payload);
        resetLegacyKttVariantStates();
        if (mode === "dao") {
          setLegacyKttDo3NamBacDaoHitMap(hitMap);
          setLegacyKttDo3NamBacDaoActive(true);
        } else if (mode === "cd") {
          setLegacyKttDo3NamBacCDHitMap(hitMap);
          setLegacyKttDo3NamBacCDActive(true);
        } else {
    ...
    async function runLegacyKttDoBacNamHomSau(silent, variant) {
      var mode = normalizeLegacyKttVariantMode(variant);
      setLoading(true);
      setProgress(15);
      try {
        ...
        var hitMap = buildLegacyKttDoBacNamHomSauHitMap(sourceRows, legacyHeThong, payload.queryValue, mode);
        applyLegacyKttViewPayload(payload);
        resetLegacyKttVariantStates();
        if (mode === "dao") {
          setLegacyKttDoBacNamHomSauDaoHitMap(hitMap);
          setLegacyKttDoBacNamHomSauDaoActive(true);
        } else if (mode === "cd") {
          setLegacyKttDoBacNamHomSauCDHitMap(hitMap);
          setLegacyKttDoBacNamHomSauCDActive(true);
        } else {
    ...
    function reverseLegacyKttToken(token) {
      var t = String(token || "").trim();
      if (!t) return "";
      return t.split("").reverse().join("");
    }

    function normalizeLegacyKttVariantMode(variant) {
      var mode = String(variant || "main").trim().toLowerCase();
      if (mode === "dao" || mode === "dao" || mode === "đảo" || mode === "reverse") return "dao";
      if (mode === "cd" || mode === "cđ" || mode === "both" || mode === "chinhdao") return "cd";
      return "main";
    }

    function getLegacyKttVariantTokenCandidates(token, variant, side) {
      var base = String(token || "").trim();
      if (!base) return [];
      var mode = normalizeLegacyKttVariantMode(variant);
      var reversed = reverseLegacyKttToken(base);
      var out = [];
      var seen = {};
      ...
      if (mode === "dao") {
        var useReversed = String(side || "").toLowerCase() === "right" || String(side || "").toLowerCase() === "bac" || String(side || "").toLowerCase() === "south";
        pushCandidate(useReversed ? reversed : base);
      } else if (mode === "cd") {
        pushCandidate(base);
        pushCandidate(reversed);
      } else {
        pushCandidate(base);
      }
      return out;
    ...
    function getLegacyKttBacNamVariantCandidates(token, variant) {
      var base = String(token || "").trim();
      if (!base) return [];
      var mode = normalizeLegacyKttVariantMode(variant);
      var reversed = reverseLegacyKttToken(base);
    ...
    function kttCellNode(val, bgColor, opts) {
      ...
      var displayToken = last2;
      var inverseDoField = !!(opts && legacyKttIsInverseDoField(opts.field));
      var do3Active = !!legacyKttDo3NamBacActive || !!legacyKttDo3NamBacDaoActive || !!legacyKttDo3NamBacCDActive;
      var inverseActive = !!legacyKttDoBacNamHomSauActive || !!legacyKttDoBacNamHomSauDaoActive || !!legacyKttDoBacNamHomSauCDActive;
      var bg;
      var color;
      var fw;
      var fz;
    ...
    function kttCardNode(row) {
      ...
      var do3Active = !!legacyKttDo3NamBacActive || !!legacyKttDo3NamBacDaoActive || !!legacyKttDo3NamBacCDActive;
      var inverseActive = !!legacyKttDoBacNamHomSauActive || !!legacyKttDoBacNamHomSauDaoActive || !!legacyKttDoBacNamHomSauCDActive;
      var do3DayHits = do3Active && cardYmd ? (
        legacyKttDo3NamBacDaoActive
          ? (legacyKttDo3NamBacDaoHitMap || {})[cardYmd]
          : (legacyKttDo3NamBacCDActive ? (legacyKttDo3NamBacCDHitMap || {})[cardYmd] : (legacyKttDo3NamBacHitMap || {})[cardYmd])
      ) : null;
      var inverseDayHits = inverseActive && cardYmd ? (
        legacyKttDoBacNamHomSauDaoActive
          ? (legacyKttDoBacNamHomSauDaoHitMap || {})[cardYmd]
          : (legacyKttDoBacNamHomSauCDActive ? (legacyKttDoBacNamHomSauCDHitMap || {})[cardYmd] : (legacyKttDoBacNamHomSauEffectiveHitMap || {})[cardYmd])
      ) : null;
    ...
    function buildLegacySlrAutoTongHopSearchText(row) {
      var cText = String(buildLegacySlrAutoNumberText(row && row.latestCellData, "c") || "").trim();
      var dText = String(buildLegacySlrAutoNumberText(row && row.latestCellData, "d") || "").trim();
      var tokens = [];
      var seen = {};
      ...
      appendTokens(cText);
      if (String((row && row.mode) || "").toUpperCase() !== "2C") appendTokens(dText);

      return {
        cText: cText || "-",
        dText: String((row && row.mode) || "").toUpperCase() === "2C" ? "-" : (dText || "-"),
        searchText: tokens.join(" ")
      };
    }

    function getLegacyDd3NbQueryTypeOption(heThong) {
      var he = Number(heThong || 2) === 3 ? 3 : 2;
      return {
        value: normalizeLegacyTongHopQueryValue("D_-P_-T_-B_", he),
        text: "Đầu đuôi 3 Đài Nam - Bắc"
      };
    }
    ...
    function buildLegacySlrAutoCachItemsForQuery(searchText, queryValue, rowKey, queryIndex) {
      var rawSearch = String(searchText || "").trim();
      var fallback = [{
        key: String(rowKey || "slr_auto") + "_" + queryIndex,
        boSo: rawSearch,
        cachName: rawSearch,
    ...
    function getLegacySlrAutoSelectedRows(sourceRows) {
      var selectedKeys = legacySlrAutoSelectedRowKeys || [];
      var src = Array.isArray(sourceRows) ? sourceRows : [];
      if (!selectedKeys.length) return [];
      var selectedMap = {};
      selectedKeys.forEach(function (k) { selectedMap[String(k)] = true; });
      return src.filter(function (row, idx) {
        var key = String((row && row.key) || (String((row && row.queryValue) || "") + "_" + String((row && row.sttFrom) || "") + "_" + String((row && row.sttTo) || "") + "_" + idx));
        return !!selectedMap[key];
      });
    }
    ...
    async function runLegacySlrAutoTongHopSelectedRows() {
      ...
      if (!pickedRows.length) {
        canhbao("Vui lòng chọn ít nhất 1 dòng ở Bảng tự động để xét tổng hợp");
        return;
      }
      ...
      try {
        ...
        for (var i = 0; i < pickedRows.length; i += 1) {
          ...
          var tongHopInput = buildLegacySlrAutoTongHopSearchText(row);
          if (!String(tongHopInput.searchText || "").trim()) {
            skippedCount += 1;
            continue;
          }
        ...
        combinedRows.sort(function (a, b) {
          ...
          var bk = String(bNumbers || (b && b.tongHopInput) || "").trim();
          var dn = ak.localeCompare(bk, "vi", { numeric: true, sensitivity: "base" });
          if (dn) return dn;
          var ds = Number((a && a.sttFrom) || 0) - Number((b && b.sttFrom) || 0);
          if (ds) return ds;
          var dq = String((a && a.queryLabel) || "").localeCompare(String((b && b.queryLabel) || ""), "vi", { numeric: true, sensitivity: "base" });
          if (dq) return dq;
          return Number((b && b.ngayCXHT) || 0) - Number((a && a.ngayCXHT) || 0);
        });
        ...
        console.error(e);
        canhbao("Không thể xét tổng hợp từ các dòng SLR đã chọn");
        setLegacySlrAutoTongHopSummary("Lỗi xét tổng hợp: " + String((e && e.message) || e || "unknown"));
      } finally {
        setLegacySlrAutoTongHopRunning(false);
        setLoading(false);
        setProgress(100);
        setTimeout(function () { setProgress(0); }, 600);
      }
                          ...
                          h(Card, {
                            ...
                            h(Row, { gutter: [10, 8] }, [
                              ...
                              h(Col, { xs: 12, md: 5, key: "slrnb_from_rank" }, [
                                ...
                                h(InputNumber, themedNumberProps({ value: legacyRankFrom, min: 1, max: 93, onChange: function (v) { setLegacyRankFrom(toNumberSafe(v, 1)); } }))
                              ]),
                  ...
                  },
                  {
                    ...
                    children: h("div", { className: "kqxs-other-controls kqxs-other-controls-nb", style: { paddingTop: 4 } }, [
                      h(Row, { gutter: [12, 10], align: "middle" }, [
                        ...
                        h(Col, { xs: 24, md: 24, key: "ktt_run", style: { marginTop: 8 } }, [
                          h("div", { style: { display: "flex", flexWrap: "wrap", gap: 6, alignItems: "center", width: "100%" } }, [
                            h(Button, { type: "primary", size: "small", onClick: runLegacyKiemTraTongHop, loading: loading }, "Xem"),
                            h("span", { style: { fontSize: 12, fontWeight: 600, color: theme.textSecondary, marginLeft: 4 } }, "3 Nam → Bắc"),
                            h(Button, { size: "small", onClick: function () { runLegacyKttDo3NamBac(false, "main"); }, loading: loading }, tt.lgKttBtnDo3NamBac || "KQ 3 Nam"),
                            h(Button, { size: "small", onClick: function () { runLegacyKttDo3NamBac(false, "dao"); }, loading: loading }, tt.lgKttBtnDo3NamBacDao || "KQ 3 Nam đảo"),
                            h(Button, { size: "small", onClick: function () { runLegacyKttDo3NamBac(false, "cd"); }, loading: loading }, tt.lgKttBtnDo3NamBacCD || "KQ 3 Nam CĐ"),
                            h("span", { style: { fontSize: 12, fontWeight: 600, color: theme.textSecondary, marginLeft: 4 } }, "Bắc → Nam sau"),
                            h(Button, { size: "small", onClick: function () { runLegacyKttDoBacNamHomSau(false, "main"); }, loading: loading }, tt.lgKttBtnDoBacNamHomSau || "Lấy KQ Bắc"),
                            h(Button, { size: "small", onClick: function () { runLegacyKttDoBacNamHomSau(false, "dao"); }, loading: loading }, tt.lgKttBtnDoBacNamHomSauDao || "Lấy KQ Bắc đảo"),
                            h(Button, { size: "small", onClick: function () { runLegacyKttDoBacNamHomSau(false, "cd"); }, loading: loading }, tt.lgKttBtnDoBacNamHomSauCD || "Lấy KQ Bắc CĐ")
                          ])
                        ]),
              ...
              legacySlrAutoRows.length ? h("div", { style: { marginBottom: 10 } }, [
                ...
                h("div", { style: { display: "flex", alignItems: "center", gap: 10, marginBottom: 6 } }, [
                  ...
                  h(Button, {
                    size: "small",
                    type: "primary",
                    loading: !!legacySlrAutoTongHopRunning,
                ...
                legacySlrAutoTongHopRows.length ? h(Card, {
                  ...
                  h("div", { style: { marginBottom: 8, display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" } }, [
                    h(Button, {
                      size: "small",
                      onClick: function () { setLegacySlrAutoTongHopSelectedRowKeys((legacySlrAutoTongHopRows || []).map(function (r) { return r.key; })); }
                    }, tt.lgThAutoSelectAll || "Chọn hết"),
                    h(Button, {
                      size: "small",
                      onClick: function () { setLegacySlrAutoTongHopSelectedRowKeys([]); }
                    }, tt.lgThAutoClearAll || "Bỏ chọn"),
                    h(Button, {
                      size: "small",
                      type: "default",
                      onClick: function () {
                        exportSelectedRowsByConfig({
                          selectedKeys: legacySlrAutoTongHopSelectedRowKeys,
                          sourceRows: legacySortedRowsRef.current["slr_auto_th"] || legacySlrAutoTongHopRows,
                          keyGetter: function (row) { return String((row && row.key) || ""); },
                          columns: legacySlrAutoTongHopColumns,
                          fileName: "legacy_slr_auto_tonghop_selected",
                          sheetName: "SLR_TongHop_Auto"
                        });
                      }
                    }, "💾 Xuất đã chọn"),
                  ...
                  h(Table, {
                    ...
                    rowSelection: {
                      ...
                      columnTitle: tt.lgThSelect || "Chọn",
                      columnWidth: 32,
                      selectedRowKeys: legacySlrAutoTongHopSelectedRowKeys,
                      onChange: function (keys) { setLegacySlrAutoTongHopSelectedRowKeys(keys); }
                    },
                    columns: withMultiSortColumns(legacySlrAutoTongHopColumns),
                    dataSource: legacySlrAutoTongHopRows,
                    pagination: false,
                    size: "small",
                    scroll: { x: 2200, y: 320 },
          ...
          : legacyMainTab === "other" && activeAction === "legacy_th"
            ? h(Card, { size: "small", title: tt.lgToolTh, style: { background: theme.cardBg, color: theme.text, borderColor: theme.border } }, [
                ...
                h("div", { key: "th-auto", style: { display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 8, alignItems: "center", padding: "6px 8px", background: "color-mix(in srgb, " + theme.cardBg + " 86%, " + theme.primary + " 14%)", borderRadius: 4, border: "1px solid " + theme.border } }, [
                  ...
                  h(Input, { size: "small", placeholder: "N", value: legacyThAutoC8Tong29Min, onChange: function (e) { setLegacyThAutoC8Tong29Min(e.target.value); }, style: { width: 55 } })
                ]),
