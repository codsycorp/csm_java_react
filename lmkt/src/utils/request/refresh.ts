...
// import { fetchRefreshToken } from "#src/api/user"; // Removed - no auth for public website
...
async function fetchRefreshToken() {
	...
	const url = baseUrl ? baseUrl.replace(/\/$/, "") + "/refresh-token" : "refresh-token";
	return ky.post(url, { json: { _origin: window.location.origin } }).json<any>();
}
