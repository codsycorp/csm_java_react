...
@Service
public class CopilotMemoryManagerService {

  private static final Logger log = LoggerFactory.getLogger(CopilotMemoryManagerService.class);
  ...
  @Value("${copilot.memory-manager.context-dir:csm_datas/public}")
  private String contextDir;

  @Value("${copilot.memory-manager.max-sources:18}")
  private int maxSources;

  @Value("${copilot.memory-manager.max-summary-per-source-chars:2400}")
  private int maxSummaryPerSourceChars;

  @Value("${copilot.memory-manager.max-global-context-chars:120000}")
  ...
  public String buildAndPersistMenuGlobalContext(
    ...
    assembled.append("# Copilot Global Context\n");
  ...
  // so they fit within GitHub Models context window (~128k tokens).
  ...
  private void persistGlobalContext(String appId, String scopeKey, String text) {
    try {
      ...
      log.warn("Cannot persist copilot global context appId={} scopeKey={}: {}", appId, scopeKey, ex.getMessage());
    }
  }
