...
public class SocketIOConfig implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private final Map<UUID, String> sessionGuestLocales = new ConcurrentHashMap<>();
    private final Map<String, String> guestIdentityByAppAndPhone = new ConcurrentHashMap<>();
    private final Map<String, String> guestLocaleByKey = new ConcurrentHashMap<>();
    ...
    private String resolveGuestPreferredLocale(String appId, String guestIdentity, UUID sessionId) {
        if (appId != null && !appId.isBlank() && guestIdentity != null && !guestIdentity.isBlank()) {
            String byGuest = guestLocaleByKey.get(guestKey(appId, guestIdentity));
            if (byGuest != null && !byGuest.isBlank()) {
                return byGuest;
            }
        }
        if (sessionId != null) {
            String bySession = sessionGuestLocales.get(sessionId);
    ...
    private boolean hasHumanAdminReplyAfter(String appId, String guestIdentity, String guestPhone, Long guestMessageTimestamp) {
        ...
        long messageTs = guestMessageTimestamp == null ? 0L : guestMessageTimestamp;

        try {
            java.util.List<ChatMessage> history = chatPersistenceService.getHistoryByGuestIdentity(appId, guestIdentity, guestPhone, 40);
            ...
            for (ChatMessage msg : history) {
                ...
                if (ts == null || ts <= messageTs) {
                    continue;
                }
                if (!Boolean.TRUE.equals(msg.getIsAdmin())) {
                    continue;
                }

                String eventType = msg.getEventType();
                boolean isAutoAi = eventType != null && eventType.startsWith("ai_auto_");
                if (isAutoAi) {
                    continue;
                }

                String senderUserId = msg.getUserId();
                if (senderUserId != null && AI_ASSISTANT_USER_ID.equalsIgnoreCase(senderUserId.trim())) {
                    continue;
                }
            ...
            logger.warn("Unable to inspect admin reply status appId={}, guestIdentity={}, error={}", appId, guestIdentity, e.getMessage());
        }
        return false;
    }

    private void scheduleNoReplyFallbackAfterGuestMessage(String appId,
                                                          String guestIdentity,
                                                          String guestPhone,
                                                          String guestMessage,
                                                          Long guestMessageTimestamp,
        ...
        ScheduledFuture<?> future = chatAiScheduler.schedule(() -> {
            try {
                if (hasHumanAdminReplyAfter(appId, guestIdentity, guestPhone, guestMessageTimestamp)) {
                    logger.info("⏭️ Skip AI no-reply fallback because admin already replied - appId={}, guestIdentity={}", appId, guestIdentity);
                    return;
                }
    ...
    private void dispatchAiMessageToGuest(String appId,
                                          String guestIdentity,
                                          String guestPhone,
                                          String messageText,
                                          String eventType,
        ...
        String safeMessageText = sanitizeGuestWelcomeText(messageText, eventType, preferredLocale);
        if (safeMessageText.isBlank()) {
            safeMessageText = fallbackWelcomeByLocale(preferredLocale);
        }
        ...
        adminAlert.setLocale(normalizeLocale(preferredLocale));
    ...
    private void triggerWelcomeOnGuestJoin(String appId, String guestIdentity, String guestPhone, String preferredLocale) {
        ...
        Future<?> future = chatAiScheduler.schedule(() -> {
            try {
                ...
                dispatchAiMessageToGuest(appId, guestIdentity, guestPhone, fallbackWelcome, "ai_auto_welcome_fallback", normalizedLocale);
    ...
    public void init() {
        ...
        server.addDisconnectListener(client -> {
            ...
            sessionGuestLocales.remove(sessionId);
        ...
        server.addEventListener("join", ChatMessage.class, (client, data, ackSender) -> {
            ...
            String guestLocale = normalizeLocale(data.getLocale());
            ...
            if (isAdmin != null && isAdmin) {
                ...
                if (guestLocale != null && !guestLocale.isBlank()) {
                    sessionGuestLocales.put(sessionId, guestLocale);
                    guestLocaleByKey.put(guestKey(appId, guestIdentity), guestLocale);
                }
                ...
                String preferredLocale = resolveGuestPreferredLocale(appId, guestIdentity, sessionId);
                triggerWelcomeOnGuestJoin(appId, guestIdentity, guestPhone, preferredLocale);
            ...
            try {
                ...
                if (ackSender != null) ackSender.sendAckData("{\"success\":false,\"error\":\"" + e.getMessage() + "\"}");
                            ...
                            server.addEventListener("chat", ChatMessage.class, (client, data, ackSender) -> {
                                ...
                                String guestPhone = data.getGuestPhone();
                                String guestSessionId = data.getGuestSessionId();
                                String preferredLocale = normalizeLocale(data.getLocale());
                                String appId = data.getAppId();
                                ...
                                if (guestContext) {
                                    ...
                                    if (guestIdentity != null && !guestIdentity.isEmpty()) {
                                        ...
                                        rememberGuestPhoneIdentity(appId, guestPhone, guestIdentity);
                                        String localeToRemember = firstNonBlank(preferredLocale, resolveGuestPreferredLocale(appId, guestIdentity, client.getSessionId()));
                                        localeToRemember = normalizeLocale(localeToRemember);
                                        if (localeToRemember != null && !localeToRemember.isBlank()) {
                                ...
                                if (userId == null && guestIdentity != null && !guestIdentity.isEmpty()) {
                                    String resolvedLocale = normalizeLocale(firstNonBlank(preferredLocale, resolveGuestPreferredLocale(appId, guestIdentity, client.getSessionId())));
                                    if (resolvedLocale != null && !resolvedLocale.isBlank()) {
                                        data.setLocale(resolvedLocale);
                                    }
