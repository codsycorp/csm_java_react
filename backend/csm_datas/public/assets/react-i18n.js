...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.default = Object.assign({}, _components2.default, _lib2.default);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _DEFAULT_PROP_TYPES, _PROP_WHITELIST;
	...
	var LOCALE_ATTR = 'data-i18n';

	var DEFAULT_PROP_TYPES = (_DEFAULT_PROP_TYPES = {
	  id: _react.PropTypes.string
	}, _defineProperty(_DEFAULT_PROP_TYPES, LOCALE_ATTR, _react.PropTypes.string.isRequired), _defineProperty(_DEFAULT_PROP_TYPES, 'options', _react.PropTypes.object), _defineProperty(_DEFAULT_PROP_TYPES, 'fallback', _react.PropTypes.string), _defineProperty(_DEFAULT_PROP_TYPES, 'className', _react.PropTypes.string), _defineProperty(_DEFAULT_PROP_TYPES, 'onClick', _react.PropTypes.func), _DEFAULT_PROP_TYPES);
	...
	var PROP_WHITELIST = (_PROP_WHITELIST = {
	  id: 'id',
	  className: 'className',
	  onClick: 'onClick',
	  href: 'href',
	  target: 'target'
	}, _defineProperty(_PROP_WHITELIST, LOCALE_ATTR, LOCALE_ATTR), _defineProperty(_PROP_WHITELIST, 'type', 'type'), _defineProperty(_PROP_WHITELIST, 'for', 'htmlFor'), _defineProperty(_PROP_WHITELIST, 'value', 'value'), _PROP_WHITELIST);

	var RENDERER = _lib2.default.RENDERER;

	var DEFAULT_ELEM = {
	  componentWillMount: function componentWillMount() {
	    var _this = this;

	    var localeKey = this.props[LOCALE_ATTR];
	    this.destroyUpdate = _lib2.default.onUpdate(localeKey, function () {
	      _this.updateMessage(_this.props);
	    });
	  ...
	  updateMessage: function updateMessage(props) {
	    var localeKey = props[LOCALE_ATTR];
	    var message = _lib2.default.getMessage(localeKey);
	  ...
	  filterProps: function filterProps() {
	    ...
	    Object.keys(this.props).forEach(function (prop) {
	      var propKey = PROP_WHITELIST[prop];
	      if (propKey) {
	        props[propKey] = _this2.props[prop];
	      }
	    });
	    return props;
	  },
	  render: function render() {
	    ...
	    return _react2.default.createElement(
	      state.tagName,
	      props,
	      _lib2.default.renderI18n(state.message, this.props.options, this.renderer)
	    );
	  }
	};

	var i18n = {};
	var VALID_TAGS = ['p', 'span', 'a', 'strong', 'button', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'label', 'option'];
	VALID_TAGS.forEach(function (tagName) {
	  i18n[tagName] = _react2.default.createClass(Object.assign({
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _fbemitter = __webpack_require__(161);
	...
	var i18nBundles = {};
	var storePrefix = 'i18n.' + window.location.host;
	...
	*/
	function setConfig(conf) {
	  Object.keys(conf || {}).forEach(function (prop) {
	    var oldValue = CONFIG[prop];
	    var newValue = conf[prop];
	    CONFIG[prop] = conf[prop];
	    if (prop === 'lang') {
	      var bNames = Object.keys(i18nBundles);
	      var cache = getCache(CONFIG.lang) || {};
	      setBundles(cache);

	      load(bNames).then(function () {
	...
	*/
	function loadSync(langBundles) {
	  if (langBundles) {
	    Object.keys(langBundles).forEach(function (lang) {
	...
	*/
	function loadBundlesSync(lang, langBundles) {
	  loadSync(_defineProperty({}, lang, langBundles));
	}
	...
	function getCache(lang) {
	  if (store) {
	    var item = store.getItem(storePrefix + '.' + lang);
	...
	function loadBundleAsync(localeKey) {
	  ...
	  var _parseLocaleKey = parseLocaleKey(localeKey);

	  var _parseLocaleKey2 = _slicedToArray(_parseLocaleKey, 1);

	  var bName = _parseLocaleKey2[0];
	  ...
	  if (!CONFIG.url) {
	    var message = 'Set the i18n URL path to asynchronously load ' + bName + ' bundle.';
	    return Promise.reject(new Error(message));
	  }

	  // fetch bundle!
	  var url = normalizeURL(CONFIG.url + '/' + CONFIG.lang + '/' + bName + '.' + CONFIG.ext);
	  i18nBundles[bName] = fetch(url).then(function (resp) {
	    return resp.ok ? resp.json() : Promise.reject();
	  }).then(function (bMessages) {
	    var cache = getCache(CONFIG.lang) || {};
	    setCache(CONFIG.lang, Object.assign(cache, _defineProperty({}, bName, bMessages)));
	    i18nBundles[bName] = templatize(bMessages);
	  }, function () {
	    var message = bName + ' bundle failed to load.';
	...
	*/
	function load(bundleNames) {
	  ...
	  var keys = typeof bundleNames === 'string' ? [bundleNames] : bundleNames;
	  var bNames = dedup(keys.map(function (localeKey) {
	    var _parseLocaleKey3 = parseLocaleKey(localeKey);

	    var _parseLocaleKey4 = _slicedToArray(_parseLocaleKey3, 1);

	    var bName = _parseLocaleKey4[0];

	    return bName;
	  }));
	  return Promise.all(bNames.map(function (bName) {
	...
	function getMessage(localeKey) {
	  if (typeof localeKey === 'string') {
	    var _parseLocaleKey5 = parseLocaleKey(localeKey);

	    var _parseLocaleKey6 = _slicedToArray(_parseLocaleKey5, 2);

	    var bName = _parseLocaleKey6[0];
	    var bKey = _parseLocaleKey6[1];
	...
	*/
	function renderI18n(localeKey, options) {
	  var render = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : renderString;
	...
	*/
	function onUpdate(localeKeys, callback) {
	  ...
	  var listener = emitter.addListener(EVENTS.LANG_CHANGE, callback);
	  return function () {
	    listener.remove();
	  };
	}
	...
	*/
	function batchRenderI18n(localeKeys, options) {
	  var map = {};
	  forEach(localeKeys, function (localeKey, ref) {
	    if ((typeof localeKey === 'undefined' ? 'undefined' : _typeof(localeKey)) === 'object') {
	...
	exports.default = {
	  ...
	  batchRenderI18n: batchRenderI18n,
	  RENDERER: {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var fbemitter = {
	  ...
	  EmitterSubscription : __webpack_require__(163)
	};
